...
public class IntArrayFrontCodedList extends AbstractObjectList<int[]> implements Serializable, Cloneable {
 ...
 public IntArrayFrontCodedList( final Iterator<int[]> arrays, final int ratio ) {
  ...
  while( arrays.hasNext() ) {
   ...
   if ( n % ratio == 0 ) {
    ...
    array = IntArrays.grow( array, curSize + count( length ) + length, curSize );
    ...
    array = IntArrays.grow( array, curSize + count( length ) + count( common ) + length, curSize );
    curSize += writeInt( array, length, curSize );
 ...
 /** Reads a coded length.
	 ...
	 */
 private static int readInt( final int a[], int pos ) {
 ...
 /** Writes a length.
	 ...
	 * @return the number of elements coding <code>length</code>.
	 */
 private static int writeInt( final int a[], int length, int pos ) {
  ...
  return new AbstractObjectListIterator<int[]>() {
    ...
    public int[] next() {
     ...
     if ( i % ratio == 0 ) {
      pos = p[ i / ratio ];
      length = readInt( array, pos );
      a = IntArrays.ensureCapacity( a, length, 0 );
      ...
      if ( inSync ) {
       length = readInt( array, pos );
       common = readInt( array, pos + count( length ) );
       a = IntArrays.ensureCapacity( a, length + common, common );
       System.arraycopy( array, pos + count( length ) + count ( common ), a, common, length );
       pos += count( length ) + count( common ) + length;
       length += common;
      }
      else {
       a = IntArrays.ensureCapacity( a, length = length( i ), 0 );
       extract( i, a, 0, length );
      }
     }
     i++;
     return IntArrays.copy( a, 0, length );
    }

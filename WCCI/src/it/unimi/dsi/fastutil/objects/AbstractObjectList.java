...
public abstract class AbstractObjectList <K> extends AbstractObjectCollection <K> implements ObjectList <K>, Stack <K> {
 ...
 private final class MyIter extends AbstractObjectListIterator <K> {
    ...
    public final boolean hasNext() { return pos < AbstractObjectList.this.size(); }
    public final boolean hasPrevious() { return pos > 0; }
    public final K next() { if ( ! hasNext() ) throw new NoSuchElementException(); return AbstractObjectList.this.get( last = pos++ ); }
    public final K previous() { if ( ! hasPrevious() ) throw new NoSuchElementException(); return AbstractObjectList.this.get( last = --pos ); }
 ...
 public static final class ObjectSubList <K> extends AbstractObjectList <K> implements java.io.Serializable {
  ...
  private final class MyIter extends AbstractObjectListIterator <K> {
     ...
     public final boolean hasPrevious() { return pos > 0; }
     public final K next() { if ( pos >= size() ) throw new NoSuchElementException(); return l.get( from + ( last = pos++ ) ); }
     public final K previous() { if ( pos <= 0 ) throw new NoSuchElementException(); return l.get( from + ( last = --pos ) ); }
     public final int nextIndex() { return pos; }
     public final int previousIndex() { return pos - 1; }
     public final void add( K k ) {
      if ( last == -1 ) throw new IllegalStateException();
  ...
  public final void addElements( int index, final K a[], int offset, int length ) {
   int sz = to - from;
     if ( index < 0 ) throw new IndexOutOfBoundsException( "Index (" + index + ") is negative" );
     if ( index >= sz ) throw new IndexOutOfBoundsException( "Index (" + index + ") is greater than or equal to list size (" + ( sz ) + ")" );
  ...
  public final ObjectList <K> subList( final int from, final int to ) {

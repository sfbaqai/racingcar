...
public class ObjectLinkedOpenCustomHashSet <K> extends AbstractObjectSortedSet <K> implements java.io.Serializable, Cloneable, Hash {
 ...
 public ObjectLinkedOpenCustomHashSet( final int n, final float f, final Strategy<K> strategy ) {
 ...
 public ObjectLinkedOpenCustomHashSet( final int n, final Strategy<K> strategy ) {
 ...
 private class SetIterator extends AbstractObjectListIterator <K> {
  ...
  SetIterator( K from ) {
   if ( ( strategy.equals( (key[ last ]), (from) ) ) ) {
    ...
    next = first;
    K k;
    do k = next(); while( ! ( strategy.equals( (k), (from) ) ) );
    curr = -1;
   }
  }

...
//		}
	...
	private static final String[] headers = {"Segment (type = ",TAB+"num = ",TAB+"radius = ",TAB+"length = ",TAB+"start = ",TAB+"end = ",TAB+"center = ",TAB+"dist = "," )"};	
	...
	private static final char[] dista = headers[7].toCharArray();			
	private static final char[] fin = headers[8].toCharArray();
...
//						double ssx = sx + dx*d;
						...
						if (prev.end.y<startY){
							if (ssy>=startY || ssy<=prev.end.y) continue;
						} else if (prev.start.y>endY){
							if (ssy>=prev.start.y || ssy<=endY) continue;
						}
...
//		int[][][] allTurn = (inTurn) ? (which==1) ? CircleDriver2.allTpR : CircleDriver2.allTpL : null;
		...
		if (size==2){
			...
			if (prev!=null && e>E){				
				...
				if (last.y-first.y<3 && s.type!=Segment.UNKNOWN && s.type!=0 && s.radius-s.type*tW<=25)
					return indx;
			}
			
			if (s.type==Segment.UNKNOWN || (s.type!=0 && (s.end.y-s.start.y>=s.radius || s.radius<=REJECT_VALUE || s.radius>=MAX_RADIUS-1 || s.radius-2*s.type*tW<=REJECT_VALUE || s.radius-s.type*tW>=MAX_RADIUS-1))) {									
				return indx;
			}
			if (s.type!=Segment.UNKNOWN){
				s.startIndex = from;
...
//				seg.opp.done = oldDone;
	...
	public static int remove(Segment s,int which,int currentIndx,int trSz){
		...
		if (!s.unsafe){
			Segment next = null;
			for (int i = currentIndx+1;i<trSz;++i){
				int j = trIndx[i]; 
				Segment t = trArr[ j ];
				if (t.unsafe || i==trSz-1) {
					if (!t.unsafe) {
					...
					if (i-currentIndx-1>0) {
						if (trSz-i>0) System.arraycopy(trIndx, i, trIndx, currentIndx+1, trSz-i);
						trSz-=i-currentIndx-1;
					}
			...
			if (s!=null && s.type!=Segment.UNKNOWN) {					
				...
				if (s!=null && s.type!=Segment.UNKNOWN && next!=null && next.type!=Segment.UNKNOWN &&  next.start.y<=s.end.y+SMALL_MARGIN && next.end.y>s.end.y+SMALL_MARGIN){
					...
					if (next.type!=Segment.UNKNOWN)
						...
						if (currentIndx==trSz-1) next = null;
					}
				...
				if (s.updated){					
					...
					if (s.num>2){
						if (!ok && fst!=null && lst!=fst){
							...
							Vector2D center = tmpCenter; 
							circle(v[s.startIndex], v[s.endIndex], s.center.x,s.center.y, s.radius,center);
							double d1 = check(v, firstIndex,lastIndex, center, s.radius);
							ok = (d1>=0);
							if (ok) {
								s.center.copy(center);
		...
		if ((from<s.startIndex || to>s.endIndex+1) && to-from>1){
			...
			for (int i = from;i<=s.startIndex;++i){
				...
				if (isConfirm){
					...
					d-=s.radius;
					if (d<0) d=-d;
					if (d>EPS || (s.center.y==0 && d>E) ) continue;
				}
...
//			}
			...
			if (i>0 && pl!=null && pl.type==l.type && l.type!=0 && (pl.radius==l.radius && Math.abs(pl.center.y-l.center.y)<1)){
				pl.end.copy(l.end);
...
//					pr.updated = false;
			...
			if (prev!=null && (t.type==0 && Math.abs(t.end.x-t.start.x)<=E && t.end.y-t.start.y>3 && firstSeg.type!=0) || (t.type!=0 && t.center.y==0  && firstSeg.end.y<1 && (firstSeg.type==0 || firstSeg.radius!=t.radius))){
				boolean good = true;
				if (t.type!=0){
	...
	public String toString() {
		...
		System.arraycopy(dista, 0, buf, len, dista.length);
		len+=dista.length;		
		len = double2string(this.dist,buf,len);

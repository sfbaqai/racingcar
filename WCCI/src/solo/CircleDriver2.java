...
public final class CircleDriver2{
	 ...
	 * 
	 */
	public static final double BREAK_TIME = 5700.40; 
	//		661.28;
...
//				acc = (relativePosMovement>0 || absSpeedY<absLastSpeedY) ? CONSTANT_SPEED_ACC*0.25 : CONSTANT_SPEED_ACC*0.25;
			...
			if (dist-W>speedRadius) {
				if (edgeDetector.highestPoint!=null && edgeDetector.highestPoint.length()>=EdgeDetector.MAX_DISTANCE || certain || dist-W*1.5>speedRadius || absSpeedY<MODERATE_SPEEDY){
					...
					if (relativePosMovement<-0.01 && relativeAngle<-0.1)
						acc *= CONSTANT_SPEED_ACC;
					else if (relativePosMovement<-0.01 && relativeAngle<-0.01 && relativeAngleMovement<-0.001) acc*=INCREASE_ONE;
...
//					acc = INCREASE_ONE;
				...
				} else acc = (absSpeedY<10) ? dist>speedRadius ? 1 : CONSTANT_SPEED_ACC*0.25 : 0;
...
//			steer = (-turn*relativeTargetAngle/steerLock+ gotoPoint(cs, 0, 1))*0.5;
		...
		if (!flyingHazard){
			...
			} else if (steer*turn>0 && relativeTargetAngle<-0.001 && relativeAngle>0.1 && relativePosMovement>0.01)
				steer = turn;

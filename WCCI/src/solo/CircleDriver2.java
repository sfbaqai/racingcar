...
public final class CircleDriver2{
	 ...
	 * 
	 */
	public static final double BREAK_TIME = 2300.64; 
	//		661.28;
		...
		if (absSpeedY>VERY_HIGH_SPEEDY && relativePosMovement<-0.001){
			...
			if (speedX>targetSpeed+FAST_MARGIN){
				...
				steer =  relativeAngleMovement>0.02 ? turn : (relativeAngleMovement>0.01) ? bal : (relativeAngleMovement>0.001 && relativeAngleMovement>lastRelativeAngleMovement) ? bal*0.5 
						: relativeAngleMovement<0.001 ? -turn : 0;
				
				brake = (relativeAngleMovement>0.01) ? 1 
						: (relativeAngleMovement>0 || steer*turn>=0) ? (speedX*speedX-targetSpeed*targetSpeed)/(speedX*speedX) 
						: (speedX*speedX-targetSpeed*targetSpeed)*0.5/(speedX*speedX);
			} else if (speed>targetSpeed+gap){
				acc = 0;						
				steer = relativeAngleMovement>0.02 ? turn : (relativeAngleMovement>0.01) ? bal : (relativeAngleMovement>0.001 && relativeAngleMovement>lastRelativeAngleMovement) ? bal*0.5 
						: relativeAngleMovement<0.001 ? -turn : 0;
				brake = speedX>targetSpeed+Math.max(15,toOutterEdge*3) && relativeAngleMovement>0.01 ? 1  
					: (relativeAngleMovement>0 || steer*turn>=0) ? (speedX*speedX-targetSpeed*targetSpeed)/(speedX*speedX) 
							: (speedX*speedX-targetSpeed*targetSpeed)*0.5/(speedX*speedX);
			}
			else {
//				acc = 0;
				acc = (speed>targetSpeed+gap || relativeAngleMovement<-0.001 && relativeAngle<0.005 || relativeAngleMovement<-0.01 || relativeAngle>0.35 || relativeAngleMovement>-0.001 && relativeAngle>0.3 || relativeAngle>0.25 && relativeAngleMovement>0.01) ? CONSTANT_SPEED_ACC*0.25 : 1;
				brake = 0;
...
//				steer = turn;
					...
					steer = (relativeAngleMovement>0.02 || relativeAngleMovement>0.015 && relativeAngleMovement>lastRelativeAngleMovement) 
							...
							: relativeAngleMovement>0.01 && relativeAngleMovement>lastRelativeAngleMovement 
								...
								: relativePosMovement<-0.01 && relativeAngleMovement>lastRelativeAngleMovement && relativeAngleMovement>0.001 || relativeAngleMovement>0.01 
									? bal 
									: (relativeAngleMovement<0.001) ? -turn : steer;
				}
			...
			if (relativeAngleMovement<0 && brake==0 && speed>targetSpeed){
				acc = 0;
				brake = (speed*speed-targetSpeed*targetSpeed)/(speed*speed);
			}
			if (relativeAngleMovement>0 && (relativeAngle>0 || relativeAngleMovement>0.01) && speed<targetSpeed+gap)
				acc = 1;
//			steer =0;
			return steer;
		}
		...
		if (absSpeedY>=50){		
			...
			if (relativeAngleMovement>0 && (relativeAngle>0 || relativeAngleMovement>0.01) && speedX<targetSpeed+gap)
				acc = 1;
			return acc;
		}
...
//				acc = 0;
			if (acc>0 && (isSafeToAccel || m>20) && relativePosMovement<-0.001 && distToEstCircle<0) 
				acc = CONSTANT_SPEED_ACC*0.25;
			brake = 0;
			return acc;
		}
...
//			if (relativePosMovement<-0.001 && relativeAngle>0) hazard = 2;
			if (steer*lastSteer>=0 && relativeSpeedY>0 && absSpeedY<=MODERATE_SPEEDY) {
				boolean noAdjustSteering = (distToEstCircle>0 && relativePosMovement>-0.001 && Math.abs(steer)<Math.abs(lastSteer));
...
//					acc = 0;
					...
					acc = 0;
				else if (!isSafeToAccel && m<20 && distToEstCircle<-GAP && (relativeAngleMovement<-0.001 && distToEstCircle<-W || relativeAngleMovement<-0.01) && relativePosMovement<0.001 && speedX>highestSpeed+m)
					acc = 0;
...
//							steer = (steer*turn<0) ? -turn : (steer==0) ? 0 : turn;
							...
							? relativePosMovement<-0.02 && relativeAngleMovement<-0.001 ? brake : 0 
							: brake;
						...
						if (speed>=targetSpeed+15 && ((isSafeToAccel || steer*turn>0) && toInnerEdge<-W*1.5)) 
							brake = (isSafeToAccel && steer*turn<0 || absSpeedY>=MODERATE_SPEEDY || distToEstCircle<-W) ? brake : 1;
								...
								? (distToEstCircle<-W*1.2 && !isSafeToAccel) 
										? brake 
										: relativeAngleMovement>-0.01 && (seenNewSeg && (absSpeedY<MODERATE_SPEEDY || absSpeedY<absLastSpeedY) || speed<targetSpeed+15 && distToEstCircle>lastDistToEstCircle && absSpeedY<MODERATE_SPEEDY) 
											? (seenNewSeg && highest!=null && highest.y<50 && distToEstCircle<-GAP*0.5 && distToEstCircle<lastDistToEstCircle) ? brake : brake*0.5 
											: brake 
...
//						(mLastY>0 && speed>=targetSpeed+Math.min(mLen,5)) ? brake : 0;
					if ((absSpeedY>HIGH_SPEEDY || absSpeedY>=MODERATE_SPEEDY && relativeAngleMovement<0 && relativePosMovement<0 && relativeTargetAngle>0) && speed>targetSpeed+FAST_GAP || mLastY>0 && speed>=targetSpeed+FAST_GAP && !mustTurn || speed>=targetSpeed+FAST_GAP && relativeSpeedY<0 && !canGoToLastSeg){ 
						...
						if (mustTurn && relativePosMovement>=0 && !canGoToLastSeg) {
							...
							brake = (absSpeedY<MODERATE_SPEEDY || steer*turn>0 && relativeAngleMovement<0.01) ? 1 : brake;
//							steer = 0;
						}
					} else if (speed>targetSpeed+Math.max(10,mLen*0.5)){ 
...
//					if (lastSeg!=null && (lastSeg.type==0 || speedRadius<lastSeg.radius)) angle*=0.5;
					...
					if (a<0 && steer*turn>0){
						if (absSpeedY>MODERATE_SPEEDY && (relativeAngleMovement<-0.01 || relativeAngleMovement<-0.001 && relativeAngleMovement<lastRelativeAngleMovement)){
							steer *= 0.5;
							hazard = 2;
...
//							if (speed>maxSpeed && steer*turn>0) steer=0;
							...
							steer = (relativeAngleMovement<-0.01 || relativeAngleMovement<lastRelativeAngleMovement
									|| b>TURNANGLE && speedX>maxSpeed && absSpeedY>HIGH_SPEEDY && relativeAngleMovement<0.01 && absSpeedY<absLastSpeedY
									|| b>TURNANGLE*0.75 && speedX>maxSpeed+10 && absSpeedY>MODERATE_SPEEDY && m>15) ? 0 : steer*0.5;
...
//					} else 
					if (b>TURNANGLE && speedX>maxSpeed && a>0 && steer*turn>0)
						...
						steer = (absSpeedY<10 && distToEstCircle>-W) ? maxAbs(steer,lSteer) 
								: (relativeAngleMovement<-0.001 || relativeAngleMovement<0.01 && relativeAngleMovement<lastRelativeAngleMovement) 
									? -turn 
									: relativeAngleMovement>0.02 ? 0 : relativeAngleMovement>0.01 ? steer*0.5 : steer;
						...
						steer = (speedX<lowestSpeed-15 && a>0 && distToEstCircle>-tW) ? minAbs(steer,lSteer*0.5) : (m>30 && speedX<MODERATE(lowestSpeed, m) && a<TURNANGLE) ? relativeAngle>0 ? 0  : maxAbs(steer,lSteer*0.5) : 
							(distToEstCircle<-W || (distToEstCircle<-GAP || distToEstCircle<0 && lastSteer*turn<0 && relativeAngleMovement<-0.001) && isDanger || b>TURNANGLE && a>TURNANGLE/1.5 && distToEstCircle<-GAP || b>TURNANGLE && isDanger && speedX>maxSpeed+20 && distToEstCircle<0) 
								? distToEstCircle<lastDistToEstCircle 
										? (relativeAngleMovement<0.001 || relativeAngleMovement<0.01 && relativeAngleMovement<lastRelativeAngleMovement) ? -turn : steer 
										: (lastSteer+steer)*0.5
								: steer;
					...
					if ((m<20 || a>TURNANGLE*0.75) && relativeSpeedY>0 && isFast && a>TURNANGLE*0.5 && b>TURNANGLE && relativePosMovement>0 && distToEstCircle<-GAP){
						steer = (m<20 || m<25 && distToEstCircle<-GAP) 
								? (relativeAngleMovement<-0.001 || relativeAngleMovement<0.01 && relativeAngleMovement<lastRelativeAngleMovement) 
										? -turn 
										: relativeAngleMovement>0.02 ? 0 : relativeAngleMovement>0.01 ? steer*0.3 : steer 
								: maxAbs(lSteer,steer);
					} else
...
//			steer = (-turn*relativeTargetAngle/steerLock+ gotoPoint(cs, 0, 1))*0.5;
		...
		if (!flyingHazard){
			...
			if (relativeAngle<-0.001 && relativePosMovement<-0.01 && toOutterEdge<=GAP)
			...
			} else if (relativePosMovement<-0.001 && relativeAngleMovement<-0.001 && speedX>expectedSpeed){
				...
				steer = (distToEstCircle>0 && relativeAngleMovement>0) ? steer : relativeAngle>0 && absSpeedY<MODERATE_SPEEDY && absSpeedY<absLastSpeedY && (seenNewSeg || trSz>1 && last_speed>first_speed 
						|| lastAcc>CONSTANT_SPEED_ACC && speedX<last_speed+m*2 || speedX<last_speed+Math.max(m,tW) && distToEstCircle>-tW) ? maxAbs(steer,maxAbs(lastSteer, steer)+turn*0.06) : (relativeSpeedY>0) ? -turn : steer;
				...
				if (m<10 && relativePosMovement>0.01 && distToEstCircle>0 && relativeAngleMovement>-0.01){
					...
					steer = (distToEstCircle>lastDistToEstCircle && (m>15 || !isFast)) ? (toInnerEdge<-GAP) ? steer : 0 : 
						(m<10 && isFast && speedX>highestSpeed+tW*2 && (toInnerEdge<-GAP || distToEstCircle<0)) ? -turn : relativeAngleMovement<0 ? maxAbs(steer,lastSteer) : steer;
...
//						maxSpeed = Math.min(speedX,last_speed);
					...
					(canGoToLastSeg) ? (relativePosMovement>0 && (last_speed>first_speed || last_speed>105 || seenNewSeg && highestPoint!=null && highestPoint.y>50)) 
							? faster 
							: targetSpeed+m*2-tW :
		...
		: (relativeAngle>0.15) 
			?  relativeAngleMovement>0.01 
					? toOutterEdge>-W && relativePosMovement<0 
							? relativeAngleMovement<lastRelativeAngleMovement && relativeAngleMovement<0.015 
									? deflect 
									: relativeAngleMovement>0.02 ? -signD : relativeAngleMovement>0.015 ? 0 : 0
					: relativeAngleMovement<lastRelativeAngleMovement 							 
						? relativeAngleMovement>0.02 ? -signD : relativeAngleMovement>0.015 ? toOutterEdge>-W ? 0 : bal*0.5 : deflect 
			...
			: relativeAngleMovement<-0.001 && relativePosMovement>0 || relativeAngleMovement<lastRelativeAngleMovement  
				? relativeAngleMovement<0.01 || relativeAngleMovement<lastRelativeAngleMovement ? signD : (relativeAngleMovement<0.015) ? deflect : 0 
				: relativeAngleMovement<0.01 || relativeAngleMovement<lastRelativeAngleMovement ? signD : (relativeAngleMovement<0.015) ? deflect : 0  
...
//						}
						...
						if (speedX>50 && (relativeAngleMovement<0.001 && steer*deflect>0 && relativeAngle<0.2 || relativeAngle<0.15 || relativeAngle>0.3 && relativeAngleMovement>0.001))
							acc = CONSTANT_SPEED_ACC*0.25;
						if (relativeAngle>0.2 && relativeAngle<0.3 || relativeAngle>0.25 && relativeAngleMovement<-0.001) acc = 1;
						if (acc==1 && slip>15) acc = CONSTANT_SPEED_ACC*0.5;

...
public final class CircleDriver2{
	 ...
	 * 
	 */
	public static final double BREAK_TIME = 1600.16; 
	//		661.28;
...
//			targetSpeed = lowestSpeed+FAST_MARGIN+mLen;
		...
		if ((startFlying>0 || flyingHazard || slip>20 ) && speedX<lowestSpeed && (relativeAngle>0 || maxTurn && distToEstCircle>0) && relativeTargetAngle>0) 
			steer = (relativeAngle>0) ? (relativeAngle>0.1 && absSpeedY>=MODERATE_SPEEDY && relativeAngleMovement>0.001) ? Math.atan2(speedY,speedX) : 0 : steer*0.5;
		...
		if (flyingHazard || speedX>50 && speedX<lastSpeedX && slip>10 && slip>0 && lastSlip>0 && balance>0){
			if ((flyingHazard && !landed || startFlying>0 && relativeAngle>0 && relativeAngleMovement>0.001 && steer*turn<0 && speedX<lowestSpeed) && absSpeedY<MODERATE_SPEEDY && relativeTargetAngle>0)
				...
				steer = (relativeAngle>0.2 && relativeAngleMovement>0 && absSpeedY>HIGH_SPEEDY) ? Math.signum(speedY) : (flyingHazard) ? steer*0.5 : (relativeAngle<-0.01 || relativeTargetAngle<0 || relativePosMovement<-0.001 && relativeAngleMovement<-0.01) ? steer*0.5 : (absSpeedY>absLastSpeedY+4 || absSpeedY>MODERATE_SPEEDY && absSpeedY>absLastSpeedY+2) ? curAngle/steerLock 			
				...
				if (absSpeedY>=MODERATE_SPEEDY && relativeAngle>0 && relativeAngleMovement>0.001 && relativeTargetAngle>0 && (flyingHazard || speedX<lowestSpeed)){
					steer = (absSpeedY>HIGH_SPEEDY) ? turn : (steer*turn<=0) ?  Math.atan2(speedY, speedX) : maxAbs(Math.atan2(speedY, speedX), steer);
				} else if (absSpeedY>=MODERATE_SPEEDY && relativeAngle<0 && relativeAngleMovement<-0.001 && (flyingHazard || speedX<lowestSpeed))
					steer = (absSpeedY>=HIGH_SPEEDY || relativePosMovement<-0.01) ? -turn : steer;
				else if ((relativePosMovement<-0.01 || relativePosMovement<0 && relativeAngleMovement<-0.01) && (!landed || relativeAngleMovement<-0.01)) steer = -turn;
			}
...
//						if (!isSafeToAccel && absSpeedY>=MODERATE_SPEEDY) brake = Math.max(brake,0.15);							
						...
						brake = speed>=targetSpeed+10 && (isSafeToAccel && relativeAngle<0 || isSafeToAccel && distToEstCircle<-GAP && distToEstCircle<lastDistToEstCircle || relativePosMovement>-0.001 && (distToEstCircle>-(W*1.5) || relativeAngle<0)) ? (speed*speed-targetSpeed*targetSpeed)/(speed*speed) : 0; 
					else brake = 0;
...
//						(mLastY>0 && speed>=targetSpeed+Math.min(mLen,5)) ? brake : 0;
					if ((absSpeedY>HIGH_SPEEDY || absSpeedY>=MODERATE_SPEEDY && relativeAngleMovement<0 && relativePosMovement<0 && relativeTargetAngle>0) && speed>targetSpeed+FAST_GAP || mLastY>0 && speed>=targetSpeed+FAST_GAP && !mustTurn || speed>=targetSpeed+FAST_GAP && relativeSpeedY<0 && !canGoToLastSeg){ 
						...
						if (mustTurn && relativePosMovement>=0 && !canGoAtCurrentSpeed) {
							brake = (speed<targetSpeed+FAST_MARGIN) ? 0 : Math.min(brake, 0.1);
...
//		}
		//*/		
			...
			if (relativePosMovement>0 && relativeTargetAngle>0 && relativeAngle>0 && relativeAngleMovement>0 && distToEstCircle>-GAP && lastDistToEstCircle<distToEstCircle) {
...
//		slip = speedX/3.6- slip/4.0f;
		...
		if (!flyingHazard && speedX>50 && (speedX<lastSpeedX || startFlying>0 && speedX<lastSpeedX+0.1) && slip>10 && slip>0 && lastSlip>0 && balance>0){
			if (startFlying==0) 
				startFlying = time;
...
//			return (m>40) ? targetSpeed+15+tW+1.5*m : (m>30) ? targetSpeed+15+tW+m : (m>15) ? targetSpeed+15+tW+0.5*m : targetSpeed+15+tW+0.35*m;
	...
	private double deltaX(double mmx,double mmy,double m,double lowestSpeed,Segment lastSeg,double a,double b, double absSpeedY){
		if (m>35 && (distToEstCircle>-W*1.5 && b<TURNANGLE*0.75 || distToEstCircle>-W) && (speed<lowestSpeed+Math.max(m, FAST_MARGIN)*2 || distToEstCircle>=0 && b<TURNANGLE|| b<TURNANGLE*0.5)){
			return (m>35 && distToEstCircle<-GAP) ? lastSeg.type*(Math.max(tW-lgap,W)) :  lastSeg.type*(tW*2-lgap-GAP);
			...
			return (m<35 && b<TURNANGLE*0.75 && distToEstCircle>-W) ? lastSeg.type*(Math.max(tW*2-lgap-GAP,W)) : lastSeg.type*(Math.max(tW-lgap,W));
		} else if (m>20 || distToEstCircle>=0){
			if ((canGoToLastSeg || distToEstCircle>=0) && absSpeedY<=10)
				return (m>20) ? lastSeg.type*W : lastSeg.type*GAP*1.5;
...
//					} else 
					...
					if ((relativePosMovement<-0.01 || relativeAngleMovement<-0.01 && relativePosMovement<0) && speedX>first_speed && !flyingHazard) steer = -turn;
					return steer;
...
//					return steer;
			...
			if (isFast && !isOffBalance && relativeTargetAngle>0 && m<10 && relativePosMovement>0 && relativePosMovement<0.01 && relativeAngleMovement<0 && relativeAngleMovement<lastRelativeAngleMovement && relativePosMovement<lastRelativePosMovement)
				steer = -turn;
			else steer = ((isFast || relativeAngle<-0.001 && speedX>lowestSpeed-tW) && (a>TURNANGLE*0.5 && m<10 && !canGoToLastSeg || relativePosMovement>0 && slip>0 && !canGoToLastSeg && m<10 || m<5 && a>0 && relativeAngleMovement<-0.001)) ? -turn : steer;
...
//			steer = (-turn*relativeTargetAngle/steerLock+ gotoPoint(cs, 0, 1))*0.5;
		...
		if (!flyingHazard){
			if (relativeAngle<-0.001 && relativePosMovement<-0.01 && toOutterEdge<=GAP)
				...
				steer = -turn;
			else if (m<20 && relativeSpeedY>0 && isFast && absSpeedY>MODERATE_SPEEDY && a>0 && b>TURNANGLE && relativePosMovement>0){
				steer = (toInnerEdge<-GAP || distToEstCircle<-GAP) ? -turn : (-turn+steer)*0.5;
				...
				if (relativeSpeedY<-LOW_SPEEDY && absSpeedY<absLastSpeedY && relativeTargetAngle>0) {
					...
					steer =(relativeSpeedY>0) ? maxAbs(lastSteer, steer)*1.5 : maxAbs(lastSteer, steer);
			...
			} else if (steer*turn>0 && relativeTargetAngle<-0.001 && relativeAngle>0.1 && relativePosMovement>0.01 && a<0)
				steer = turn;
		...
		if (Math.abs(toMiddle)>=tW){
			...
			if (flyingHazard && !landed){				
					steer = (absSpeedY<MODERATE_SPEEDY) ? 0 : 
						(absSpeedY<HIGH_SPEEDY || absSpeedY<absLastSpeedY) ? Math.atan2(speedY, speedX)*0.5 : (steer*turn<0) ? -turn : turn;				
				acc = 1;
				brake = 0;
				return steer;
			}
			if (tp!=0 && tp!=Segment.UNKNOWN && tp!=2){
				if (toInnerEdge<W   && isFast && (relativeAngleMovement<-0.01 || relativeAngleMovement<-0.001 && absSpeedY<absLastSpeedY || relativePosMovement<0.001 && (relativeAngle<0 || relativePosMovement<-0.001))){
					steer = -tp;
					acc = 0;
					if (speedX<=maxSpeed){
...
//					if (relativeAngle<-0.001 && toInnerEdge<=GAP*0.5) acc = 1;
					...
					steer = (relativePosMovement>0.01 && relativeAngle>0.1 || toInnerEdge>W && relativePosMovement>0.001) ? curAngle*0.5/steerLock : -tp;
					acc = (isFast) ? CONSTANT_SPEED_ACC*0.5 : 1;
//					if (relativeAngle<-0.001 && toInnerEdge<=GAP*0.5) acc = 1;
	//				if (speedX<=maxSpeed){
	//					if (speedX<=maxSpeed-1.5)
	//						acc = 1;
	...
	//				acc = Math.max(acc,CONSTANT_SPEED_ACC);
					...
					steer = toInnerEdge>W ? (relativeAngle<0) ? curAngle*0.5/steerLock : 0 : (relativePosMovement<0)? 0 : (relativeAngle>=0) ? curAngle*0.5/steerLock : 0;
					acc = 1;
					brake = 0;
					return steer;
				}
				...
				if (turn!=2 && turn!=Segment.UNKNOWN){
					if (tp==0)
						...
						: (relativePosMovement<0.001 && absSpeedY<MODERATE_SPEEDY) 
							? deflect 
							: (relativeAngle>0.15 || relativeAngle>0.1 && relativePosMovement>0 && absSpeedY>=MODERATE_SPEEDY && relativeAngleMovement>0.001) ? Math.atan2(speedY, speedX)*0.5 : (relativeAngle<0.05) ? deflect : 0;
					else if (toInnerEdge<0){
						if (relativePosMovement<-0.001 && relativeAngle>0 && relativeAngle<0.15 && (absSpeedY>absLastSpeedY && absSpeedY>10 || absSpeedY>MODERATE_SPEEDY || relativeAngleMovement<-0.001)) acc = Math.max(CONSTANT_SPEED_ACC*0.5,minAbs(acc, lastAcc)*0.95);
						steer = (relativeAngle>0.15) ? (relativeAngleMovement>0.03) ? 2*curAngle/steerLock : (relativeAngleMovement>0.01) ? Math.atan2(speedY, speedX) : 0 
								: (relativeAngle>0.1) 
										? (relativePosMovement<-0.001) ? Math.signum(deflect) : (relativeAngleMovement>0 && relativePosMovement>0.001) ? 0 : deflect 
										: (relativePosMovement<-0.001 || relativeAngleMovement<-0.001) ? Math.signum(deflect) : (relativeAngleMovement<-0.001 && lastSteer*deflect>0) ? maxAbs(deflect, lastSteer) : deflect;
					} else steer = (relativeAngle<-0.15 && relativeAngleMovement<-0.001) ? -Math.signum(deflect) : (relativeAngle<-0.1) ? Math.atan2(speedY, speedX)*0.5 :(relativeAngle<-0.05 || absSpeedY>=MODERATE_SPEEDY && relativeAngle<0) ? 0 : lastSteer*deflect>0 && relativeAngle>=0 ? maxAbs(lastSteer, deflect) : deflect;
				} else {
...
//				if (steer*deflect>0) steer = deflect*0.5;
				if (steer*turn<=0 && relativeAngle>=0 && (flyingHazard || startFlying>0)){
					steer = (absSpeedY<MODERATE_SPEEDY && relativePosMovement>0.001) ? 0 : (relativeAngle>0.2 || relativePosMovement>0.001 && relativeAngle>0.1 && relativeAngleMovement>0) ?
						Math.atan2(speedY, speedX)*0.5 : steer; 
				}
...
//					acc *= 0.4;
			...
			if (absSpeedY>=MODERATE_SPEEDY || absSpeedY>Math.abs(lastSpeedY)+4){
				acc = (relativeAngle>0) ? 1 : 0.4;
				if (absSpeedY>HIGH_SPEEDY || absSpeedY>Math.abs(lastSpeedY)+4) 
					steer = (relativeAngle>0 && (relativeAngle<0.15 || relativeAngleMovement<-0.001)) ? 0 : Math.signum(speedY);
...
//			double relativeSpeedY = tp*speedY;
		...
		if (turn==STRAIGHT){
			...
			steer = (absSpeedY>=MODERATE_SPEEDY || relativeAngle>0.15) ? Math.atan2(speedY,speedX) : lSteer+deflect;
			return steer;
		}

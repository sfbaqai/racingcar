...
/*
 * A really, really simple FDbigint package
...
class FDBigInteger {
	...
	public FDBigInteger( FDBigInteger other ){
		data = new int[nWords = other.nWords];
		System.arraycopy( other.data, 0, data, 0, nWords );
	}

	private FDBigInteger( int [] d, int n ){
		data = d;
		nWords = n;
	}

	public FDBigInteger( long seed, int digit[], int nd0, int nd ){
	...
	mult( int iv ) {
		...
		if ( p == 0L){
			return new FDBigInteger( r, nWords );
		} else {
			r[nWords] = (int)p;
			return new FDBigInteger( r, nWords+1 );
		}
	}
	...
	add( FDBigInteger other ){
		...
		if ( c != 0L ){
			...
			s[i++] = (int)c;
			return new FDBigInteger( s, i );
		}
		return new FDBigInteger( r, i );
	}
	...
	sub( FDBigInteger other ){
		...
		return new FDBigInteger( r, n-nzeros );
	}

	private static boolean dataInRangeIsZero(int i, int m, FDBigInteger other) {
		while ( i < m )
			if (other.data[i++] != 0)
				return false;
		return true;
	 ...
	 */
	public int
	quoRemIteration( FDBigInteger S )throws IllegalArgumentException {

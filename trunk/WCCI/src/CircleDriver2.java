...
public final class CircleDriver2{
	 ...
	 * 
	 */
	public static final double BREAK_TIME = 254.64 ;
...
//		if (acc>0 && maxTurn && trSz==2 && last.type==0)
//			return acc;
		...
		if (!inTurn && acc>0 && lowestSpeed<180 && (speed>=200 && speed>=lowestSpeed+FAST_MARGIN && lowestSeg.center.y<SAFE_DISTANCE))
			...
			//			double tmpAcc = acc;
			if (tp*turn<0){				
				if (speed>150 && speed>=lowestSpeed+FAST_MARGIN && my>=FAST_MARGIN){
					acc = 0;
				...
				if (tp==turn && speed>200 && lowestSegIndx>0 && speed>=lowestSpeed+FAST_MARGIN && my<=FAST_MARGIN){
					acc = 0;
					if (relativeSteer>0) 					
						steer = (!canGoAtCurrentSpeed) ? -turn : (relativeAngle>0) ? 0 : (curAngle*turn>0) ? 0 : curAngle/steerLock;
				} else if (tp==turn && trSz==1 && (relativeAngle<-0.01 || speed>=lowestSpeed+tW && (absSpeedY>=MODERATE_SPEEDY || relativeCurPos>=1)) && (relativePosMovement<-0.001 || relativeAngleMovement<-0.001 || relativeTargetAngle<-0.001 || absSpeedY>absLastSpeedY) && toInnerEdge<=-W*0.5){
					acc = 0;
					...
					if (absSpeedY>MODERATE_SPEEDY && toOutterEdge>=tW && relativeAngle>=0)
						steer = 0;
					else if (absSpeedY>=HIGH_SPEEDY) steer = turn; 
					acc = 0;					
				} else if (tp==turn && relativePosMovement<-0.001 && relativeAngle<-0.01  && relativeTargetAngle<-0.001 && relativeAngleMovement<-0.001 && toOutterEdge<=W*4.5)
					acc = 0;
					...
					acc = 0;
				else if (tp==turn && absSpeedY>=MODERATE_SPEEDY && (absSpeedY>absLastSpeedY || relativePosMovement<-0.001 || relativeAngleMovement<-0.001) && speed>=lowestSpeed+tW ) 
					acc = 0;
		...
		if (relativeTargetAngle<-0.01 && steer*turn>0){
			if (mustPassPoint.y>FAST_MARGIN){
				//				steer *= (speed>200) ? 0.1 : (speed>150) ? 0.5 : 1;
			} else {
				steer *= (speed>200) ? 0 : (speed>150) ? 0.1 : (speed>100) ? 0.3 : 1;
			}
		...
		if (speedX<250 && canGoAtCurrentSpeed && speedX>targetSpeed && lowestSpeed+FAST_MARGIN>=speedX && curPos*tp>0.2 && lastSteer*tp<=0 && carDirection.x*tp>=0){
		...
		if (drift || tooFast || toOutterEdge<=W*0.5 && relativePosMovement<=-0.001 || absSpeedY>MODERATE_SPEEDY || toOutterEdge<SAFE_EDGE_MARGIN && speedX>lowestSpeed && relativeSteer<-0.001 && (relativePosMovement<-0.001 || relativeAngle<0 ) ) {			
					...
					if (relativeHeading>=0.01){
						if (absSpeedY>=HIGH_SPEEDY){
							if (absSpeedY>absLastSpeedY || relativeSpeedY<0)
								steer = (relativeSpeedY>0) ? -sign : sign;
							else steer = 0;
							//							if (toOutterEdge<=W*0.5 && acc>0 && relativeAngle<-0.001) acc = 0;
			...
			gear = Math.max(getGear(speed),lowestGear);
			gear = (int) ((tooFast) ? (relativeHeading<0) ? gear : Math.min(lowestGear,lastGear) : (targetSpeed<speed || acc==0 || speed<lowestSpeed+FAST_MARGIN && (absSpeedY>MODERATE_SPEEDY || absSpeedY>absLastSpeedY)) ?  Math.max(gear,lastGear) : gear);
			...
			if (absSpeedY<HIGH_SPEEDY && (relativeTargetAngle>=-0.001 && relativeTargetAngle<0.1 || (Math.abs(carDirection.x)<0.05 && relativeCurPos>0.35 & speedX>first_speed+FAST_MARGIN))){
				...
				boolean brakeHard = (relativeCurPos>0.5 || relativeTargetAngle<0.05 || (Math.abs(carDirection.x)<0.05 && relativeCurPos>0.5 && tooFast)) ? true : false;
				...
				if (!brakeHard){	
					if (absSpeedY>VERY_HIGH_SPEEDY){
						...
						if (brake>0 && brake<1 && lastSpeedX<=speedX && lastBrkCmd>0) brake = 1;
						return acc;
					}					

					if (tooFast || speedX-lowestSpeed>=FAST_MARGIN) {						
						//						gear = (relativeAnge<0 || gear>1) ? 1 : gear-1;
						if (trSz>1 && speedX-lowestSpeed-FAST_MARGIN>=Math.min(lowestSeg.start.y, 20)){
							acc = 0.0;
							brake = 1;
						} else if (trSz==1 && relativePosMovement>=-0.001 && speedX-lowestSpeed-FAST_MARGIN<20){							
							brake = 0;
						}
...
//						steer = -turn;
						...
						brake = (speedX-sp>FAST_MARGIN) ? 1 : (speedX-sp<20 && speed<first_speed) ? 0 : (speed*speed-sp*sp)/(speed*speed);
						if (brake<0) {
							acc = -brake;
							brake = 0;
						} else acc = 0;
				...
				if (relativeSpeedY>-LOW_SPEEDY && absSpeedY>=MODERATE_SPEEDY && (relativePosMovement>-0.001 || relativeAngle>=-0.001)) 
					steer = (absSpeedY<HIGH_SPEEDY) ? 0 : (relativeSteer>0) ? -steer : steer;
				...
				if (!canGoAtCurrentSpeed && speed-lowestSpeed>FAST_MARGIN){
					brake = 0;
					if (steer*turn<0) 
						steer = (absSpeedY<=MODERATE_SPEEDY) ? -turn : (absSpeedY<=HIGH_SPEEDY) ? steer*1.5 : steer;
				} 
				if (relativePosMovement>=-0.0001 ||  relativeAngleMovement>=-0.0001 ||  relativeTargetAngleMovement>=-0.0001) 
					brake = ((tooFast || speed>=lowestSpeed+FAST_MARGIN) && absSpeedY>=MODERATE_SPEEDY) ? 0.2: (tooFast || absSpeedY<=MODERATE_SPEEDY && speed>targetSpeed+FAST_MARGIN || brake>0 && !inTurn) ? Math.max(brake,0.1) : 0;
					...
					} if (tp==turn && relativeSteer>0 && absSpeedY<=MODERATE_SPEEDY && (relativeAngle>=-0.001 || relativePosMovement>=-0.001) && (relativeAngleMovement<-0.001 || absSpeedY>absLastSpeedY)){
						steer = -turn;					
				...
				if (relativeTargetAngle<TARGET_ANGLE_THRESHOLD && (relativeTargetAngleMovement>-0.0001 || relativeAngleMovement>-0.0001)){
					...
					if (relativeAngleMovement>=-0.001 || Math.abs(speedX)>absSpeedY) {
						brake = (tooFast && speedX>targetSpeed && relativeTargetAngle>2*TARGET_ANGLE_THRESHOLD) ? 1 : 0;						
					}
					//					brake = 0;
				} else {					
					//IF MOVING IN RIGHT DIRECTION OR STILL TURNING FURTHER
					if (Math.abs(relativeTargetAngle)>-TARGET_ANGLE_THRESHOLD )
						brake = speedX-targetSpeed>20 && (absLastSpeedY<absSpeedY || relativeAngle<-0.001 && relativeAngleMovement<-0.001) ? brake :0;
						else if (Math.abs(relativeTargetAngle)<-TARGET_ANGLE_THRESHOLD)
							if (absSpeedY>HIGH_SPEEDY) 
								brake = (absSpeedY>=VERY_HIGH_SPEEDY) ? (absSpeedY>=absLastSpeedY) ? 0  : brake: (brake+1)*0.5;						
							else if (relativePosMovement>=-0.0001  || relativeAngleMovement>-0.0001) 
								brake = (tooFast && speedX>targetSpeed) ? 1 : (brake>0.1) ? (brake+1)*0.5 : brake;
	...
	public final static boolean tryGotoPoint(Vector2D carDirection,double speedRadius,int turn,double px,double py,double directionx,double directiony,double dSpeed,int fromSeg,int toSeg,double[] tmp,boolean checkOptimum){
		...
		//		double relativeHeading = carDirection.x*turn;
		//		double relativePosMovement = (curPos-lastPos)*turn;
		boolean notMaxTurn = relativeAngle>0 && speed<lowestSpeed-tW || (Math.abs(speedY)<MODERATE_SPEEDY && speed<last_speed+10 && toOutterEdge>tW*0.5*W);
...
//			acc = 1;
		...
		cc.steer = steer;
		cc.meta = 0;				
		if (debug) System.out.println("Max speed  "+maxSpeed+"   Current Speed "+speed);
	...
	private final void estimateBestPath(){
		...
		if (trSz>1){				
			Segment lastS = (lastSeg.type==1) ? lastSeg.rightSeg : lastSeg.leftSeg;			
			if (!caution && trSz>=2 && lastS.type!=0 && lastS.radius<80){
			...
			if (canGo){				
				...
				//					if (mustPassPoint.y<=1) mustPassPoint.x = 0;
				centerOfTurn = null;
				cntr = (lastS.type!=0) ? new Vector2D(lastS.center.x,lastS.center.y) : null;
				rr = (lastS.type==0) ? (lowestSpeed>=speedX) ? speedRadius+1 : speedRadius-3 : lastSeg.radius;
				dx = ox - directSx;
				dy = oy-directSy;
				canGoAtCurrentSpeed = canGoAtCurrentSpeed && (Math.sqrt(dx*dx+dy*dy)>=speedRadius-2);
				//				draw = true;
			}
	...
	private final double fuzzySteering(CarState cs){
		...
		steer = (maxTurn) ? -turn : gotoPoint(cs, mustPassPoint);
		//		}
		...
		if (canGoAtCurrentSpeed && speedX>lowestSpeed+FAST_MARGIN){
			rad = speedRadius-2;
			for (int i = 0;i<trSz;++i){
				Segment t = trArr[ trIndx[i]];

				if (t.type!=0 && speedAtRadius(t.radius)<speedX-FAST_MARGIN){
					t = (turn==TURNRIGHT) ? t.rightSeg : t.leftSeg;
					//Vector2D cntr = t.center;
		...
		if ((absSpeedY<MODERATE_SPEEDY || absSpeedY<HIGH_SPEEDY && absSpeedY<absLastSpeedY) && (toInnerEdge<-W || relativeAngle<-0.001) &&  steer*turn<0 && first.type==turn && speedX>first_speed && (relativePosMovement<=-0.001 || relativeAngleMovement<=-0.001)) steer = -turn;
		...
		if (trSz>1 && first.type*turn<0 && lastSeg.type==turn && toInnerEdge<-W){			
			...
			if (dist>10 && canGoAtCurrentSpeed){//keep turning
				maxSpeed = last_speed+FAST_MARGIN+dist;
				if (speedX>first_speed-tW){					
					steer = (dist>15) ? (absSpeedY<MODERATE_SPEEDY) ? turn : (absSpeedY<HIGH_SPEEDY) ? turn*Math.min(s,Math.abs(steer)) : 0  : (relativeSpeedY<0 && steer*turn<0) ? 0 : steer;
					return steer;
				} else {
					//					lastSteer = (dist>15) ? turn*Math.min(s,Math.abs(steer)) : (absSpeedY<HIGH_SPEEDY) ? steer : 0;
					steer = (dist>15) ? speedX-dist-lowestSpeed>FAST_MARGIN || toOutterEdge<=W && speedY*turn<0 ? (dist<30 && steer*turn<0) ? -turn :-turn*Math.min(s,Math.abs(steer)) : (absSpeedY<MODERATE_SPEEDY || steer*turn>0) ? steer : 0  : (relativeSpeedY<0 && steer*turn<0) ? 0 : steer;
					return steer;
				}
			} else if (relativePosMovement>-0.001 && canGoAtCurrentSpeed){
				//				steer += (turn*relativePosMovement/(dist*EdgeDetector.steerLock));
				steer = (dist>6) ? (steer*turn<0) ? 0 : steer : steer;
			} else if (!canGoAtCurrentSpeed){
				steer = speedX>=first_speed-tW && (relativeSpeedY<-0.001 || absSpeedY<=MODERATE_SPEEDY) ? -turn :steer;
			}
		} 		
		...
		if (trSz==1 && (relativeSpeedY>0 || relativeAngleMovement<-0.001 && relativeAngle<0.001) && first.type!=0 && absSpeedY<HIGH_SPEEDY && (steer*turn<=0 || Math.abs(steer)<0.01)&& turn!=0 && speedX>first_speed-tW && (relativePosMovement<0.001 || toInnerEdge<-W)){
			steer = (canGoAtCurrentSpeed) ? mustPassPoint.y<18 && speed>=lowestSpeed+FAST_MARGIN && (relativePosMovement>-0.001 || absSpeedY<=MODERATE_SPEEDY) ? -turn : steer : (steer<0) ? -1 : (steer==0) ? 0 : 1;
			//			steer = -turn*s*0.5;
		...
		double d = (!inTurn && speed<=lowestSpeed+50 && (nextSlowSeg==null || lowestSeg==null || nextSlowSeg.radius<=lowestSeg.radius || nextSlowSeg.start.y>20) || inTurn && first_speed>=lowestSpeed+FAST_MARGIN) ? mustPassPoint.length() : !canGoAtCurrentSpeed || inTurn && speed>=lowestSpeed && first_speed>lowestSpeed && (relativePosMovement<-0.001 || relativeAngleMovement<-0.001 || absSpeedY>MODERATE_SPEEDY)|| speed>=first_speed+FAST_MARGIN && speed>=lowestSpeed+FAST_MARGIN || absSpeedY>=HIGH_SPEEDY || (trSz==1 || toOutterEdge<SAFE_EDGE_MARGIN) && (speed>first_speed+10 || relativePosMovement<-0.001 && speedX>first_speed-tW) || !inTurn && (nextSlowSeg==null || lowestSeg==null || nextSlowSeg.radius>lowestSeg.radius || nextSlowSeg.start.y<20) ? 0 : mustPassPoint.length();
		if (lowestSpeed<first_speed && lowestSeg!=null && Math.abs(lowestSpeed-last_speed)<FAST_MARGIN) targetRadius = lowestSeg.radius;
		double targetSpeed = (canGoAtCurrentSpeed) ? speedAtRadius(targetRadius) : Math.min(speedX, speedAtRadius(targetRadius));
		if (targetSpeed>highestSpeed) targetSpeed = highestSpeed;

		if (absSpeedY<HIGH_SPEEDY && speedX<lowestSpeed+FAST_MARGIN && toOutterEdge>=SAFE_EDGE_MARGIN ){
			maxSpeed = (trSz>1 && (speed>highestSpeed && lowestSeg.start.y<15 || inTurn && highestSpeed-lowestSpeed<15 &&  relativePosMovement<-0.001)) ? lowestSpeed : Math.max(targetSpeed,lowestSpeed+FAST_MARGIN)+d;
		} else maxSpeed = (!canGoAtCurrentSpeed || absSpeedY>=VERY_HIGH_SPEEDY && relativePosMovement<-0.001) ? targetSpeed+d : (speedX<first_speed && absSpeedY<=MODERATE_SPEEDY && (relativePosMovement>-0.001 || relativeAngleMovement>-0.001 && relativeAngle>0.001)) ?  targetSpeed+d+FAST_MARGIN : 
			trSz==1 || highestSpeed-lowestSpeed>15 && (toOutterEdge>SAFE_EDGE_MARGIN || relativePosMovement>=-0.001 || relativeAngleMovement>-0.001 && relativeAngle>0.001) ? targetSpeed+d+FAST_MARGIN : targetSpeed+d;
			...
			if (!inTurn && (relativePosMovement>=-0.001 || relativeAngleMovement>-0.001 && relativeAngle>0.001 && absSpeedY<MODERATE_SPEEDY))
				if (maxSpeed<targetSpeed+FAST_MARGIN) maxSpeed = targetSpeed+FAST_MARGIN;

			if (!inTurn && maxSpeed>speed && lowestSpeed<180 && (speed>=200 || speed>=lowestSpeed+FAST_MARGIN && lowestSeg.center.y<SAFE_DISTANCE))
			...
			if (absSpeedY<=MODERATE_SPEEDY && toOutterEdge>=tW && (speed<last_speed || speed<last_speed+10 && relativeAngle>-0.001)){
				...
				if (curPos*turn>0 && (lastPos-curPos)*turn<=0) maxSpeed = Math.max(maxSpeed,lowestSpeed+FAST_MARGIN+d);
				...
				if (turn!=UNKNOWN){				
					...
					if (speedX>=first_speed && speedX>=lowestSpeed+offset && lowestSegIndx>0) maxSpeed = lowestSpeed;
					if (speedX>=maxSpeed-1 && absSpeedY<=MODERATE_SPEEDY){						
						if (!inTurn && nextSlowSeg!=null && nextSlowSeg.type!=0 && nextSlowSeg.center!=null && nextSlowSeg.center.y<25 ||first.type!=0 && speedX>first_speed && speedX>lowestSpeed && first_speed<lowestSpeed) 
							d = 0;					
						if (!inTurn) 
							d*=3;
						else d*=2;
						...
						//					if (curPos*turn>0.7 && (lastPos-curPos)*turn<=0) maxSpeed = Math.max(maxSpeed,lowestSpeed+FAST_MARGIN);
	...
	private final double steerControl(CarState cs){											
		...
		if (curPos>1 || curPos<-1){
			...
			double steer = (first!=null && first.type*curPos>0 && speed>speedAtRadius(first.radius)) ? (relativeAngle>=-0.001 || relativeAngleMovement>=0.001) ? ((relativeSpeedY<-0.001 || Math.abs(speedY)<=LOW_SPEEDY)&& trSz>1 && trArr[ trIndx[trSz-1] ].type==0) ? curAngle/steerLock : -deflect : -turn-deflect : (curAngle*turn>0) ? 0 : curAngle/steerLock; 

...
public final class CircleDriver2{
	 ...
	 * 
	 */
	public static final double BREAK_TIME =220.13;
	...
	private final static int canGoDirect(Vector2D carDirection,double speedRadius,int turn,double rx,double ry,double cnx,double cny,double dSpeed,int fromSeg,int toSeg,double[] tmp){
		...
		if (turn!=0 && (first!=null && first.type==0 || speedRadius>40)){					
			...
			while (indx<szz){
				...
				//				trackData.add(ts);
				int wEdge = (startY<0 || Math.abs(startX)>40) ? -1 : isCut(cx0, cy0, rad,0,0,startX,startY, fromSeg, toSeg, tmp,W*0.5);
				if (wEdge==0){							
					if ((k>=dSpeed || k>=speedRadius) && (wEdge= isCut(tmpx, tmpy, k,startX,startY,rx,ry,fromSeg,toSeg, tmp,W*0.5))==0){						
		...
		int wEdge = pointy<=0 || speedRadius-r>Math.sqrt(pointx*pointx+pointy*pointy) ? -1 : isCut(cx, cy, r,0,0,pointx,pointy, fromSeg, toSeg, tmp,W*0.5);
		if (wEdge==0){			
...
//		if (steer*lastSteer<0 && Math.abs(steer)<0.2) steer *= 0.5;
	...
	private final static void guessTurn(){
		...
		if (trSz>0){
			...
			while (true){
				if (numL==1){
					...
					turnLeft = (lSeg.type==0 && isFirstSeg(lSeg) && Math.abs(lSeg.end.x-v.x)<E) ? 0 : (v.x>lSeg.end.x) ? 1 : -1;
				...
				if (numR==1){
					...
					turnRight = (rSeg.type==0 && isFirstSeg(rSeg) && Math.abs(rSeg.end.x-v.x)<E) ? 0 : (v.x>rSeg.end.x) ? 1 : -1;
				} else if (numR>1){
			...
			if (reCheck || (turnLeft!=0 && turnRight!=0 && turnLeft!=turnRight) || (turnLeft!=0 && turnLeft*edgeDetector.whichE>0) || (turnRight!=0 && turnRight*edgeDetector.whichE>0 && edgeDetector.highestPoint!=null && edgeDetector.highestPoint.y<60)){				
				...
				if (numL+numR>0){
					...
					double h = 0;
					Vector2D[] allPoints = new Vector2D[numL+numR+1];
					int num = 0;				
					for (int i = 0;i<numL;++i){
	...
	private final static int isCut(double ox,double oy,double radius,double startX,double startY,double endX,double endY,int fromSeg, int toSeg,double[] tmp,double W){
		...
		for (;i<toSeg;++i){
			...
			if (prev!=null && prev.upper==null){
				...
				double ntx = ey-sy;
				double nty = ex-sx;
				double n = W*0.5/Math.sqrt(ntx*ntx+nty*nty);
				sx += ntx*n;
				sy += nty*n;
				ex += ntx*n;
				ey += nty*n;
			...
			if (t.type==0){
				ntx = eY-sY;
				nty = eX-sX;
				n = W*0.5/Math.sqrt(ntx*ntx+nty*nty);
				sX += ntx*n;
				sY += nty*n;
				eX += ntx*n;
				eY += nty*n;				
	...
	private final static boolean optimalPath(double speedRadius,Segment t,Segment lastSeg,int turn,double[] tmp){
		...
		if (t.type==0 && lastSeg.type!=0){
			...
			double ntx = dy;
			double nty = dx;
			double n = turn*W*0.5/Math.sqrt(ntx*ntx+nty*nty);
			ntx*=n;
			nty*=n;
			...
			if (oy<=0 || bestRad<=speedRadius || isCut(bestCx, bestCy, bestRad, startX, startY, rx, ry, 0, trSz, tmp,W*0.5)!=0){
				Segment seg = (turn==1) ? lastSeg.leftSeg : lastSeg.rightSeg;
				rx = seg.end.x;
				ry = seg.end.y;
				...
				ntx = dy;
				nty = dx;
				n = turn*W*0.5/Math.sqrt(ntx*ntx+nty*nty);
				ntx*=n;
				nty*=n;
				rx+=ntx;
				ry+=nty;
			...
			double ndx = toMiddle+tp*(W*0.5-tW);	
			...
			if (oy<=0 || bestRad<=speedRadius || (wEdge = isCut(bestCx, bestCy, bestRad, startX, startY, rx, ry, 0, trSz, tmp,W*0.5))!=0){
				Segment seg = (turn==1) ? lastSeg.leftSeg : lastSeg.rightSeg;
				rx = seg.end.x;
				ry = seg.end.y;
				dx = rx-seg.start.x;
		...
		if (wEdge==-2) wEdge = isCut(bestCx, bestCy, bestRad, startX, startY, rx, ry, 0, trSz, tmp,W*0.5);
		if (wEdge!=0){
	...
	private static final double speedControl(CarState cs,double targetSpeed,Vector2D point){
		...
		double relativeSpeedY = turn*speedY;
		...
		double relativeTargetAngleMovement = (angle-lastTargetAngle)*turn;
		double relativeSteer = -steer*turn;		
		double relativeTargetAngle = angle*turn;
		double relativeCurPos = curPos*turn;		
...
//			targetSpeed += d;
		...
		double mLen = (trSz>1) ? (!inTurn && last.type!=0 && last.type!=Segment.UNKNOWN) ? mLastY : (canGoToLastSeg) ? mLastY : last.start.y*0.5 : (canGoToLastSeg) ? mLastY : mustPassPoint.y;
		...
		double dv = (targetSpeed-speed)/3.6;		
...
//			if (targetSpeed>lastSpeed+startY+FAST_MARGIN) targetSpeed = lastSpeed+startY+FAST_MARGIN;
			if ((toOutterEdge<=W || toOutterEdge<=tW && trSz==1 && relativeAngle<0 || trSz==1 && absSpeedY>=MODERATE_SPEEDY) && relativePosMovement<0){				
		...
		if (acc>0 && inTurn && (absSpeedY<=MODERATE_SPEEDY || maxTurn && toOutterEdge>W)&& trSz>=2 && last.type==0){
			if ((toOutterEdge> SAFE_EDGE_MARGIN || relativeAngle>-0.1|| relativeAngle>0) && maxTurn){
...
//					acc = CONSTANT_SPEED_ACC;
			...
			if (dist-W>speedRadius) {
				acc = 1;				
			} else if (dist-W*0.5<=speedRadius){
				acc = CONSTANT_SPEED_ACC;
				if (dist<=speedRadius)
					acc = 0;				
			}
		...
		boolean canGoFast = absSpeedY<=10 && relativeAngleMovement>=-0.001 && relativeAngle>-0.1 && (speed<lastSpeed && maxTurn || speed<lastSpeed+10 && relativeAngle>-0.001) && (toOutterEdge>=W || toOutterEdge>=W*0.5 && maxTurn);
		if (acc==1 && speed<lowestSpeed && absSpeedY<MODERATE_SPEEDY && toOutterEdge>W*2)
			return acc;
		if (!inTurn && acc>0 && lowestSpeed<180 && (speed>=200 && speed>=lowestSpeed+FAST_MARGIN && lowestSeg.center.y<SAFE_DISTANCE))
			acc =0;
		else if (acc>0){ 		
			//			double tmpAcc = acc;			
			if (last!=null && last.type*turn<0 || trSz==1 && tp*turn<0){				
				if (speed>200 && speed>=lowestSpeed+FAST_MARGIN && my>=FAST_MARGIN){
					acc = 0;
				...
				else if (toOutterEdge<=W*0.5 && !maxTurn && (relativePosMovement<-0.001 ||  relativeAngle<-0.001)) 
					acc = 0;
			} else if (!canGoFast){
				if (tp==turn && speed>200 && lowestSegIndx>0 && speed>=lowestSpeed+FAST_MARGIN && mLen<=FAST_MARGIN && !canGoAtCurrentSpeed && !canGoToLastSeg){
					acc = 0;
					if (relativeSteer>0) 					
						steer = (!canGoAtCurrentSpeed || lowestSeg.start.y<18) ? -turn : (relativeAngle>0) ? 0 : (curAngle*turn>0) ? 0 : curAngle/steerLock;
				} else if (tp==turn && trSz==1 && (absSpeedY>=MODERATE_SPEEDY || relativeAngle<0 && toOutterEdge<tW+W) ){ 
					if (relativeTargetAngle>0|| relativeAngle<0) acc = 0;
				} else if (tp==turn && trSz==1 && (relativeAngle<-0.01 || (relativeAngle<0 || toOutterEdge<tW+W) && speed>=lowestSpeed+tW && (absSpeedY>=MODERATE_SPEEDY || relativeCurPos>=1 || absSpeedY>absLastSpeedY)) && (relativePosMovement<-0.001 || relativeAngleMovement<-0.001 || relativeTargetAngle<-0.001 || absSpeedY>absLastSpeedY) && toInnerEdge<=-W*0.5){
					acc = (relativeAngle<0) ? 0 : CONSTANT_SPEED_ACC;
					if (speed>lowestSpeed && absSpeedY<=MODERATE_SPEEDY && relativeAngleMovement<0 && relativeTargetAngle>=0) steer = -turn;
				} else if (tp==turn && relativePosMovement<-0.001 && ((absSpeedY>MODERATE_SPEEDY && toOutterEdge<=tW+W*0.5|| toOutterEdge<W*1.75) &&  (lowestSegIndx>0 && first_speed-lowestSpeed<15 || absSpeedY>absLastSpeedY || !maxTurn && relativeAngle<=0 && relativeAngleMovement<-0.001) || toOutterEdge<W && relativeAngleMovement<-0.001 || (toOutterEdge<W*1.75 && speed>first_speed+10 && relativeAngleMovement<-0.001 &&  relativeAngle<=0 ))){
					if (absSpeedY>MODERATE_SPEEDY && toOutterEdge>=tW && relativeAngle>=0)
						steer = 0;
					else {
						if (absSpeedY>=HIGH_SPEEDY) steer = turn;
						acc = 0;
					}
										
				} else if (tp==turn && relativePosMovement<-0.001 && relativeAngle<0  && relativeAngleMovement<0 && toOutterEdge<=W*4.5){
					 if (!canGoAtCurrentSpeed && !canGoToLastSeg){ 
						 if (maxTurn) 
							 acc = CONSTANT_SPEED_ACC;
						 else acc = 0;
					 } else acc =0; 
				} else if (tp==turn && maxTurn && (absSpeedY>=MODERATE_SPEEDY && absSpeedY>absLastSpeedY ||  relativePosMovement<-0.001 &&  relativeAngle<-0.01 && relativeAngleMovement<-0.001 && toOutterEdge<=W*4)){
					if (relativeAngle>=0 && (relativeAngleMovement>-0.001 || relativePosMovement>-0.001) && toOutterEdge>=tW){ 
//						steer = 0;
						acc = 0;
					}
					if (acc>0 && absSpeedY>=MODERATE_SPEEDY) acc = 0;
//				} else if (inTurn && !maxTurn && trSz>1 && lowestSegIndx==trSz-1 && speedX>first_speed && (relativeAngleMovement<-0.001 || relativePosMovement<-0.001))
//					acc = 0;
...
//					acc = 0;
					...
					acc = 0;
				else if (tp==turn && !canGoToLastSeg && speed>lastSpeed+FAST_MARGIN ){
					acc = 0;
				}
			}
			...
			//					acc = tmpAcc;
			//				else if (relativeAngle>=-0.01 && relativeAngleMovement>=0 && toOutterEdge>W*0.5) 
			//					acc = tmpAcc;
		...
		if (steer*turn>0 && (canGoAtCurrentSpeed || canGoToLastSeg) && mLen<35 && speed>lastSpeed+Math.max(FAST_MARGIN,mLen) && relativeTargetAngle<0){
		...
		/*if (speedX<250 && turn!=Segment.UNKNOWN && turn <2 && speedX<targetSpeed && targetSpeed<safeSpeed+50 && (last!=null && trSz>0 && (last.type==0 || first.type!=0 && last.radius>first.radius)) &&(absSpeedY<MODERATE_SPEEDY || absSpeedY<HIGH_SPEEDY && (tp==turn || tp==0) && toOutterEdge<=SAFE_EDGE_MARGIN && relativeAngle<=-0.001)){
			//			if (toOutterEdge<=W*0.5 || !canGoAtCurrentSpeed) acc = 0;
			lastTargetAngle = angle;
		...
		if (acc==0){
			if (drift || tooFast || speed>targetSpeed || trSz==1 && speed>lastSpeed+FAST_MARGIN || (trSz>1 && speed>lastSpeed+FAST_MARGIN+last.start.y) || toOutterEdge<=W*0.5 && relativePosMovement<=-0.001 || absSpeedY>MODERATE_SPEEDY || toOutterEdge<SAFE_EDGE_MARGIN && speedX>lowestSpeed && relativeSteer<-0.001 && (relativePosMovement<-0.001 || relativeAngle<0 ) ) {			
				if ((speedX<=lowestSpeed || speedX<=targetSpeed && speedX<=safeSpeed)&& absSpeedY<LOW_SPEEDY && (curAngle*tp>=-0.05 && curPos*tp>-0.1)){
					if ((relativeTargetAngle>=-0.001 || relativeAngle>=-0.001) && (tooFast && nextSlowSeg.start.y>FAST_MARGIN || (last!=null && last.start.y>FAST_MARGIN))) 
						if (relativeSteer>=0 && toOutterEdge>=W*0.5) steer = 0;
					lastTargetAngle = angle;
					lastTargetSpeed = targetSpeed;		
					if (acc==0 || brake>0)gear =  Math.min(getGear(speed),lastGear);
					return acc;
				...
				if (speedX<lowestSpeed+10) {							
					...
					if (tp==turn || tp*turn==0){
						if (relativeAngleMovement<-0.001 || relativePosMovement<-0.001){ 
							//						if (toOutterEdge<=W*0.5 && acc>0 && relativeAngle<-0.001) acc = 0;
						...
						if (relativeHeading>=0.01){
							if (absSpeedY>=HIGH_SPEEDY){
								...
								if (absSpeedY>absLastSpeedY || relativeSpeedY<0){								
									...
									//								if (absSpeedY>=HIGH_SPEEDY || toOutterEdge<=W*0.5 && acc>0 && relativeAngle<-0.001) acc = 0;
									acc = CONSTANT_SPEED_ACC;
								} else steer = 0;
								if (absSpeedY>absLastSpeedY && steer*sign<0) brake = 0;								
							} 							
...
//					gear = getGear(speed+20) -1;
				...
				if (absSpeedY<HIGH_SPEEDY && (relativeTargetAngle>=-0.001 && relativeTargetAngle<0.1 || (Math.abs(carDirection.x)<0.05 && relativeCurPos>0.35 & speedX>first_speed+FAST_MARGIN))){
...
//								brake = 1;
						...
						if (tooFast || speed-lowestSpeed>=FAST_MARGIN) {						
							//						gear = (relativeAnge<0 || gear>1) ? 1 : gear-1;
							acc = 0;
							/*if (trSz>1 && speed-lowestSpeed-FAST_MARGIN>=Math.min(lowestSeg.start.y, 20)){								
								if (speed>targetSpeed+FAST_MARGIN || mLen<18 || relativeAngle<0 && relativeAngleMovement<0 && relativePosMovement<0) {
//									brake = 1;
									if (relativeTargetAngle>0) steer = -turn;
								}
							} else if (trSz==1 && relativePosMovement>=0 && relativeAngle>-0.001 && speedX-lowestSpeed-FAST_MARGIN<20){								
								brake = 0;
							}//*/
							if (speed>=lowestSpeed+FAST_MARGIN && (canGoAtCurrentSpeed ||canGoToLastSeg) || absSpeedY>=HIGH_SPEEDY){								
								acc = 0;
								if (speed>targetSpeed+Math.max(20,mLen) && mLen>10 && relativeAngle>0) 
									brake = 1;
								else if (speed>targetSpeed+Math.max(10,mLen) && relativeAngleMovement<0) 
									brake = (relativeAngle<0) ? Math.min(0.1,brake) : (speed*speed-targetSpeed*targetSpeed)/(speed*speed);
								else brake = 0;
...
//									brake = (speed*speed-targetSpeed*targetSpeed)/(speed*speed);
								...
								steer = -turn;
								if (relativeAngleMovement>0) brake = 0;
							}
	
						}
	
					} else {
						//					if (speedX>=lowestSpeed+40) {
						//						double brake = 1;
						//						if (dv>=0){													
						//							//			if (speedX<ABS_MINSPEED) return -brake;
						//							//			slip = 0.0f;
						...
						//						acc = -Math.min(brake, 1);
						//					}					
						//					double sp = (tp==0) ? 1000 : speedAtRadius(first.radius);
						//					acc = (speed>lowestSpeed+20) ? -0.3*meanSpd : (speed<lowestSpeed+15) ? 0 : -0.1*meanSpd;
						double sp = (speedX<=first_speed) ? targetSpeed : targetSpeed;
						if (relativeCurPos>0 && relativePosMovement<=0 && !tooFast && speed<=first_speed && speedX>targetSpeed && speedX<targetSpeed+Math.max(d, 15)){						
							acc = 0;
							return acc;
						}
	
						if (tooFast && absSpeedY<HIGH_SPEEDY) {
							if (speed>lowestSpeed+FAST_MARGIN+mLen && (canGoAtCurrentSpeed ||canGoToLastSeg) || absSpeedY>=HIGH_SPEEDY){
//								if (speed>lowestSpeed+FAST_MARGIN+mLen+5 && mLen<FAST_MARGIN || speed>lowestSpeed+FAST_MARGIN+mLen+20) brake = 1;
//								if (speed>targetSpeed+10 && mLen<10 || speed>targetSpeed+mLen) brake = 1;
								...
								if (relativeTargetAngle>0 && relativeAngleMovement<0) steer = -turn;
								brake = 0;
							} else {
								if (relativeAngleMovement>0) brake = 0;
								steer = -turn;
...
//							brake = (speed-targetSpeed>mLen) ? 1 : (speed-targetSpeed<10 && speed<first_speed) ? 0 : (speed*speed-sp*sp)/(speed*speed);
//							if (brake<0) {
//								acc = 0;
//								brake = 0;								
//								if (speed>targetSpeed+Math.max(FAST_MARGIN,mLen)) 
...
//							} else acc = 0;
//	
							//						brake *= (brake>0.8) ? meanSpd : 0;
							//						if (gear>1 && speed>lowestSpeed+30) gear--;
						}																						
...
//					if (F_LFT<=0 || F_RGT<=0 || Math.abs(steer)==1 && toOutterEdge>=tW+W) brake = 0;				
				} else if (relativeTargetAngle>0 && absSpeedY<HIGH_SPEEDY){					
					System.out.println("------------------------------TURN ----------------------------");			
					if (speed-lowestSpeed>FAST_MARGIN){
						if (!canGoAtCurrentSpeed){
//							brake = (!canGoToLastSeg && absSpeedY<MODERATE_SPEEDY && relativeAngle>=0 && (mLen<FAST_MARGIN || speed>targetSpeed+Math.max(FAST_MARGIN,mLen)) && mLen>=1 && relativeSpeedY>0) ? 1 : brake;
							brake = (mLastY>0 && speed>=targetSpeed+Math.max(mLen*3,FAST_MARGIN) && mLen>10) ? relativeAngle<0 ? Math.min(brake,0.1) : 1 : (mLastY>0 && speed>=targetSpeed+Math.max(mLen,10) && mLen>FAST_MARGIN && relativeAngle>=0) ? brake : 0;
	//						brake = 0;
							if (steer*turn<0) 
								steer = (absSpeedY<=MODERATE_SPEEDY && relativeTargetAngle>0 && (relativeAngle<0 || relativePosMovement<0) && (speed>200 || trSz>1 && mLastY<FAST_MARGIN)) ? -turn : (relativeSpeedY<0) ? steer : (absSpeedY<=HIGH_SPEEDY) ? steer*1.5 : steer;
							if (brake==1) 
								steer = 0;
							else if (brake==0 && !canGoToLastSeg && mLen<18)
								steer = -turn;
						} else {
//							brake = (speed*speed-lowestSpeed*lowestSpeed)/(speed*speed);
//							brake = (mLastY>0 && speed>=targetSpeed+Math.max(mLen,10)) ? relativeAngle>=0 ? brake : 1 : (mLastY>0 && speed>=targetSpeed+5 && relativeAngle>=0) ? brake : 0;
//							brake = (mLastY>0 && speed>=targetSpeed+Math.max(mLen*3,FAST_MARGIN) && mLen>10) ? relativeAngle<0 ? brake : 1 : (mLastY>0 && speed>=targetSpeed+Math.max(mLen,10) && mLen>FAST_MARGIN && relativeAngle>=0) ? brake : 0;
							brake = (mLastY>0 && speed>=targetSpeed+Math.max(mLen*3,FAST_MARGIN) && mLen>10) ? relativeAngle<0 ? Math.min(brake,0.1) : 1 : (mLastY>0 && speed>=targetSpeed+Math.max(mLen,10) && mLen>FAST_MARGIN && relativeAngle>=0) ? brake : 0;
...
//						brake = ((tooFast || speed>=lowestSpeed+FAST_MARGIN) && absSpeedY>=MODERATE_SPEEDY) ? 1: (tooFast || absSpeedY<=MODERATE_SPEEDY && speed>targetSpeed+FAST_MARGIN || brake>0 && !inTurn) ? Math.max(brake,0.1) : 0;
//						brake = (mLastY>0 && speed>=targetSpeed+Math.max(mLen,10) && mLen>10) ? relativeAngle>=0 ? brake : 1 : 0;
						brake = (mLastY>0 && speed>=targetSpeed+Math.max(mLen*3,FAST_MARGIN) && mLen>10) ? relativeAngle<0 ? Math.min(brake,0.1) : 1 : (mLastY>0 && speed>=targetSpeed+Math.max(mLen,10) && mLen>FAST_MARGIN && relativeAngle>=0) ? brake : 0;
						if (relativeSpeedY>-LOW_SPEEDY && absSpeedY<MODERATE_SPEEDY && relativePosMovement<-0.001 && speedX>targetSpeed && speedX>first_speed) 
							steer = (steer<0) ? -1 : (steer==0) ? 0 : 1;
...
//							steer = (absSpeedY<HIGH_SPEEDY) ? -turn*Math.min(s,Math.abs(steer)) : (relativeSteer>0) ? turn*Math.min(s,Math.abs(steer)) : steer;
							if (relativeAngleMovement<0) steer = (1+steer)*0.5;
							if (relativeAngle>0) acc = CONSTANT_SPEED_ACC;
						} else if (relativeSpeedY<-LOW_SPEEDY && canGoAtCurrentSpeed) 
							steer = -turn*Math.min(s,Math.abs(steer));  
	
	
						//				if (relativePosMovement<-0.001 && relativeTargetAngle>-0.001 && relativeAngleMovement>-0.001 && relativeAngle>-0.001 && relativeAngleMovement>-0.001 && speed<lowestSpeed+FAST_MARGIN) acc = 1; 
						if (tooFast && absSpeedY<MODERATE_SPEEDY && canGoAtCurrentSpeed ){
							if (relativeTargetAngle>=-0.001 || nextSlowSeg.start.y<FAST_MARGIN){ 
								brake = 1;				
							} else if (brake<0.05 && absSpeedY<MODERATE_SPEEDY) 
								brake = 0.05;
							else if (absSpeedY>HIGH_SPEEDY || absSpeedY>absLastSpeedY && absSpeedY>MODERATE_SPEEDY) {
								brake = 0;
							}
						} if (!canGoAtCurrentSpeed && tp==turn && relativeSteer>0 && absSpeedY<=MODERATE_SPEEDY && (relativeAngle>=-0.001 || relativePosMovement>=-0.001) && (relativeAngleMovement<-0.001 || absSpeedY>absLastSpeedY)){
							steer = -turn;					
						} else if (relativeSpeedY>-LOW_SPEEDY && absSpeedY<MODERATE_SPEEDY && relativeAngle>=-0.001 && (tooFast && nextSlowSeg.start.y>FAST_MARGIN || last.start.y>FAST_MARGIN && !tooFast)) steer=0;
						...
						if (brake>0) acc = 0;
						if (speed<lowestSpeed+10 && brake>0 && relativeSteer>s) brake = 0;
						if (!tooFast && speed<lowestSpeed+10 && (relativePosMovement>=-0.001 || relativeAngleMovement>=-0.001)){ 					
							brake = 0;					
						}
//						if (!canGoAtCurrentSpeed && !canGoToLastSeg && absSpeedY>MODERATE_SPEEDY && speed>targetSpeed+10 && (relativeAngle>0 || toOutterEdge>tW+W)){
//							brake = 1;
//						}
//						if (brake==1) steer = -turn;
//						if ((F_LFT<=0 || F_RGT<=0) && steer!=0) brake = 0;								
				} else {
					System.out.println("------------------------------DURING DRIFT ----------------------------");							
					//IF pANGLE TO TARGET EXCEEDS THRESHOLD AND STILL TURNING, RECOVER
					if (relativeTargetAngle<TARGET_ANGLE_THRESHOLD && (relativeTargetAngleMovement>-0.0001 || relativeAngleMovement>-0.0001)){
						...
						if (!tooFast && relativeAngleMovement<-0.0001) steer = (lastSteer==0 || relativeAngle<-0.001) ? -sign : 0;
						if (relativeAngleMovement>=-0.001 || Math.abs(speedX)>absSpeedY) {
//							brake = (speed>targetSpeed+Math.max(mLen,FAST_MARGIN)) ? 1 : 0;
//							brake = (speed>targetSpeed+Math.max(mLen,FAST_MARGIN)) ? 1 : (!canGoToLastSeg || absSpeedY>=MODERATE_SPEEDY) ? brake : 0;
							if (speed>targetSpeed+Math.max(mLen,20) && mLen>10) 
								brake = 1;
							else if (speed>targetSpeed+mLen || absSpeedY>=MODERATE_SPEEDY && speed>targetSpeed) 
								brake = (speed*speed-targetSpeed*targetSpeed)/(speed*speed);
							else brake = 0;
						}
						//					brake = 0;
					} else {					
						//IF MOVING IN RIGHT DIRECTION OR STILL TURNING FURTHER
						if (Math.abs(relativeTargetAngle)>-TARGET_ANGLE_THRESHOLD )
//							brake = speed-targetSpeed>20 && (absSpeedY>HIGH_SPEEDY || absLastSpeedY<absSpeedY || relativeAngle<-0.001 && relativeAngleMovement<-0.001) ? 1 :0;
							brake = (speed>targetSpeed+Math.max(mLen,10)) ? 1 : (!canGoToLastSeg || absSpeedY>=MODERATE_SPEEDY) ? brake : 0;
						else if (Math.abs(relativeTargetAngle)<-TARGET_ANGLE_THRESHOLD){
							if (absSpeedY>HIGH_SPEEDY) 
								brake = (absSpeedY>=VERY_HIGH_SPEEDY) ? (absSpeedY>=absLastSpeedY) ? 0  : 1: (brake+1)*0.5;						
							else if (relativePosMovement>=-0.0001  || relativeAngleMovement>-0.0001) 
//								brake = (tooFast && speed>targetSpeed) ? 1 : (brake>0.1) ? (brake+1)*0.5 : brake;
//								brake = (speed>targetSpeed+Math.max(mLen,10)) ? 1 : (!canGoToLastSeg || absSpeedY>=MODERATE_SPEEDY) ? brake : 0;
								if (speed>targetSpeed+Math.max(mLen,20) && mLen>10) 
									brake = 1;
								else if (speed>targetSpeed+mLen || absSpeedY>=MODERATE_SPEEDY && speed>targetSpeed) 
						...
						if (brake==0 && relativeTargetAngle<TARGET_ANGLE_THRESHOLD && Math.abs(speedX)<absSpeedY) 
							steer = (relativeAngleMovement>=-0.0001 && (absSpeedY>=VERY_HIGH_SPEEDY || absSpeedY>=HIGH_SPEEDY && absAngle>0.5)) ? -sign : (relativeCurPos>0.4  || absSpeedY>=HIGH_SPEEDY && steer*sign<0) ? steer :  steer;
						else steer = (absSpeedY>=HIGH_SPEEDY && (brake>0 || relativeAngleMovement>-0.001 && relativeAngle>=-0.001)) ? 0 : (relativeAngleMovement>-0.001) ? (absSpeedY>=HIGH_SPEEDY) ? -sign : (steer*sign<0) ? steer : -sign*s	: steer;
						if (absSpeedY<MODERATE_SPEEDY && relativeSteer>-0.001 && (tooFast || relativePosMovement<-0.001)) steer = (steer<0) ? -1 : (steer==0) ? 0 : 1;
						//					if (relativeAngleMovement<-0.0001) steer = (lastSteer==0 || relativeAngle<-0.001) ? -sign : 0;
						//ELSE
...
//						steer = turn;
//						brake = 0;
//					} else if (absSpeedY<absLastSpeedY && relativeTargetAngle>0 && relativePosMovement<0){
//						steer = -turn;
//						brake = 0;
////						acc = CONSTANT_SPEED_ACC;
//					}
...
//			steer = -sign;			
		...
		} else if (absSpeedY>=HIGH_SPEEDY || absSpeedY>=MODERATE_SPEEDY && absSpeedY-absLastSpeedY>2.5 && relativeAngle>=0){
			if (speed<targetSpeed){
				acc = 1;
				brake = 0;
				steer = (absSpeedY>=VERY_HIGH_SPEEDY || absSpeedY-absLastSpeedY>2.5) ? turn : 0;
			} else {
		...
		if (brake==0){
			...
			if (relativeAngleMovement<0 && relativeTargetAngle>0){
				if (absSpeedY<absLastSpeedY && absSpeedY>10) steer = -turn;						
			} 
			if ((absSpeedY>=MODERATE_SPEEDY || relativeAngleMovement>0 && (canGoAtCurrentSpeed || canGoToLastSeg)) && absSpeedY>absLastSpeedY){ 
				acc = 0.5;				
			} 
			if (relativePosMovement>0 && relativeAngleMovement>0 && relativeAngle>0 && relativeTargetAngle>=0 && acc>0){
				 acc = 1;
...
//		} 
	...
	public final static boolean tryGotoPoint(Vector2D carDirection,double speedRadius,int turn,double px,double py,double directionx,double directiony,double dSpeed,int fromSeg,int toSeg,double[] tmp,boolean checkOptimum){
		...
		//		double relativeHeading = carDirection.x*turn;
		//		double relativePosMovement = (curPos-lastPos)*turn;
		boolean notMaxTurn = relativeAngle>0 && speed<lowestSpeed-tW || (Math.abs(speedY)<MODERATE_SPEEDY && lastSeg.type!=0 && speed<last_speed+10 && toOutterEdge>tW*0.5*W);

		if (t.type!=0 && t.type*turn>=0 && !notMaxTurn){
			double nx = turn*carDirection.y;	
			double ny = -turn*carDirection.x;			
			double ox = nx*speedRadius;
			double oy = ny*speedRadius;
			...
			lx = ox + lx*l;
			ly = oy + ly*l;
			if ((toInnerEdge<-W || toInnerEdge<-W*0.5 && relativeAngle<-0.001) && speedX>=Math.min(first_speed,last_speed)-tW && speedX<first_speed && speedX<last_speed){
				wEdge = isCut(ox, oy, speedRadius,0,0,lx,ly, fromSeg, toSeg, tmp,W*0.75);
				if (wEdge==0){
					double dx = -curPos*0.5;
					double dy = 1;
					tmp[0] = ox;
					tmp[1] = oy;
					tmp[2] = speedRadius;
					...
					lx = ox + lx*l;
					ly = oy + ly*l;
					if (isCut(ox, oy, sp,0,0,lx,ly, fromSeg, toSeg, tmp,W*0.75)==0 && (Math.abs(speedY)<=MODERATE_SPEEDY || Math.abs(speedY)<=HIGH_SPEEDY && isCut(ox, oy, sp,0,0,lx,ly, fromSeg, toSeg, tmp,W*0.75)==0)){
						tmp[0] = ox;
						tmp[1] = oy;					
						tmp[2] = sp;
						tmp[3] = lx;
				...
				//			}
				//				wEdge = isCut(ox, oy, speedRadius,0,0,lx,ly, fromSeg, toSeg, tmp,W*0.5);
				//				if (wEdge==0){				
				//					tmp[3] = carDirection.x;
				//					tmp[4] = carDirection.y;					 
				//				} else {				
				//					tmp[0] = ox;
				//					tmp[1] = oy;
				...
				//					tmp[2] = speedRadius-1;
				//					tmp[3] = 0;
				//					tmp[4] = Math.max(1,lastSeg.start.y);
				//				}	
				//				if (tr<=lr){
				//					tmp[0] = t.center.x;
				//					tmp[1] = t.center.y;
				//					tmp[2] = (toOutterEdge>=W) ? t.radius : speedRadius;
				//				} else if (lastSeg.type!=0){
				//					tmp[0] = lastSeg.center.x;
				//					tmp[1] = lastSeg.center.y;
				//					tmp[2] = (toOutterEdge>=W) ? lastSeg.radius : speedRadius;;
				//				}

			} else if ((toInnerEdge<-W || toInnerEdge<-W*0.5 && relativeAngle<-0.001) && (speedX>first_speed || speedX>last_speed || Math.abs(speedY)>MODERATE_SPEEDY)){				
				wEdge = isCut(ox, oy, speedRadius,0,0,lx,ly, fromSeg, toSeg, tmp,W*0.75);
				if (wEdge==0){														
					double sp = speedRadius+1;
					ox = nx*sp;
					...
					lx = ox + lx*l;
					ly = oy + ly*l;
					if (isCut(ox, oy, sp,0,0,lx,ly, fromSeg, toSeg, tmp,W*0.5)==0 && (Math.abs(speedY)<=MODERATE_SPEEDY || Math.abs(speedY)<=HIGH_SPEEDY && isCut(ox, oy, sp,0,0,lx,ly, fromSeg, toSeg, tmp,W*0.75)==0)){									
						tmp[2] = sp;						
					} else tmp[2] = speedRadius;
					tmp[0] = ox;
					tmp[1] = oy;	
					tmp[3] = lx;
					tmp[4] = ly;
					maxTurn = true;
					return true;
				} else if ((toInnerEdge<-W || toInnerEdge<-W*0.5 && relativeAngle<-0.001) && speedX>=first_speed && wEdge!=turn){
					double sp = speedRadius-1;
					ox = nx*sp;
					oy = ny*sp;					
					lx = lastSeg.end.x - ox;
					...
					lx = ox + lx*l;
					ly = oy + ly*l;
					//					if (isCut(ox, oy, sp,0,0,lx,ly, fromSeg, toSeg, tmp,W*0.5)==0){
					tmp[0] = ox;
					tmp[1] = oy;					
					tmp[2] = sp;
					tmp[3] = lx;
			...
			for (int j= trSz-1;j>=0;--j){
				...
				if (!DANGER && py>35){
					...
					if (toMiddle*turn<0 || carDirection.x*turn<0 || speedRadius>100){		 				
						tmp[3] = (s.radius>=120 || s.radius>100 && t.type!=0) ? (turn*toMiddle>0 && s.radius<120) ? toMiddle : toMiddle-s.type*(0.5*W-tW) : toMiddle+s.type*(0.5*W-tW);
						tmp[4] = py;
					} else {
						tmp[3] = lastSeg.end.x;
						tmp[4] = lastSeg.end.y;
				...
				if (s.type!=0){
					...
					if (oy<=0 || bestRad<=speedRadius || (isCut(bestCx, bestCy, bestRad, startX, startY, rx, ry, 0, trSz, tmp,W*0.5))!=0){						
						...
						if (cutSeg!=null && cutSeg.type!=0){
							...
							ok1 = bestRad>speedRadius && isCut(bestCx, bestCy, bestRad, 0, 0, rx, ry, 0, trSz, tmp, W*0.5)==0;
						}
					}
					...
					boolean ok2 = isCut(bCx, bCy, bRad, 0, 0, rx, ry, 0, trSz, tmp, W*0.5)==0;
					if (ok2){
						if (!ok1 || bRad>bestRad){
							bestCx = bCx;
							bestCy = bCy;
		...
		for (int i =trSz-1;i>=1;--i){
			...
			if (cutSeg!=null && cutSeg.type!=0){
				...
				if (Geom.ptTangentLine(0, 0, cutSeg.center.x, cutSeg.center.y,  rad ,tmpBuf)==0){
					rad = cutSeg.radius-tW+W*0.5;
					if (Geom.ptTangentLine(0, 0, cutSeg.center.x, cutSeg.center.y, rad ,tmpBuf)==0){
				...
				if (sz!=0){
					...
					if (bestRad>=lowestRad && isCut(bestCx, bestCy, bestRad, 0, 0, rx, ry, 0, trSz, tmp, W*0.5)==0){
		...
		if (lastSeg.type==0 || directRad>=lastSeg.radius){
			...
			directSx = tmp[0];
			directSy = tmp[1];				
			wEdge=(speedRadius-directRad>Math.sqrt(directSx*directSx+directSy*directSy)) ? -1 : isCut(directCx, directCy, directRad,0,directCy,px,py, fromSeg, toSeg, tmp,W*0.5);
			if (wEdge==0){				
				if (directSy<=0){
	...
	public final static boolean tryMaxTurn(Vector2D carDirection,double speedRadius,int turn){
		...
		if (relativeAngle>0 && speed<lowestSpeed-tW || (Math.abs(speedY)<MODERATE_SPEEDY && speed<last_speed+10 && toOutterEdge>tW*0.5*W)) return false;

		if (t.type!=0 && t.type*turn>=0){
			double nx = turn*carDirection.y;	
			double ny = -turn*carDirection.x;			
			double ox = nx*speedRadius;
			double oy = ny*speedRadius;
			...
			ly = oy + ly*l;
			if ((toInnerEdge<-W || toInnerEdge<-W*0.5 && relativeAngle<-0.001) && speedX>=Math.min(first_speed,last_speed)-tW && speedX<first_speed && speedX<last_speed){
				wEdge = isCut(ox, oy, speedRadius,0,0,lx,ly, 0, trSz, tmp,W*0.5);
				if (wEdge==0){
					double dx = -curPos*0.5;
					double dy = 1;					
					...
					lx = ox + lx*l;
					ly = oy + ly*l;
					if (isCut(ox, oy, sp,0,0,lx,ly, 0, trSz, tmp,W*0.5)==0 && (Math.abs(speedY)<=MODERATE_SPEEDY || Math.abs(speedY)<=HIGH_SPEEDY && isCut(ox, oy, sp,0,0,lx,ly, 0, trSz, tmp,1)==0)){											
						tmp[2] = sp;
						tmp[3] = lx;
						tmp[4] = ly;
					} else {
			...
			} else if ((toInnerEdge<-W || toInnerEdge<-W*0.5 && relativeAngle<-0.001)){				
				wEdge = isCut(ox, oy, speedRadius,0,0,lx,ly, 0, trSz, tmp,W*0.5);
				if (wEdge==0){													
					double sp = speedRadius+1;
					ox = nx*sp;
					...
					maxTurn = true;
					return true;
				} else if ((toInnerEdge<-W || toInnerEdge<-W*0.5 && relativeAngle<-0.001) && wEdge!=turn){
					double sp = speedRadius-1;
					ox = nx*sp;
					oy = ny*sp;					
					lx = lastSeg.end.x - ox;
	...
	static final double verify(Vector2D[] v,int to,Vector2D[] opoints,int otherTo,Segment edge,int wh,double toMiddle,double tW,double nraced){	
		...
		if (edge!=null && sz>0){
			...
			tp = (angle<0) ? -1 : 1;		
			if (r>Segment.MAX_RADIUS-1 && edge.type!=Segment.UNKNOWN) {
				...
				if (first!=null && first.type!=0 && first.center!=null && (first.center.y!=0 || first.end.y<=nraced)&& ((l0!=null && l0.type==0 && l0.start!=null && r0.start!=null && Math.abs(l0.start.x-l0.end.x)<0.1*TrackSegment.EPSILON) || (r0!=null && r0.type==0 && r0.start!=null && r0.end!=null && Math.abs(r0.start.x-r0.end.x)<0.1*TrackSegment.EPSILON))){
					...
					if (ok1 && ok2) {
						...
						for (int i = 0;i<sndIndx;++i)
							occupied[ trIndx[i] ] = 0;
						if ((trSz-=sndIndx)>0) System.arraycopy(trIndx, sndIndx, trIndx, 0, trSz);
					}
					...
					if (l0.end.y-l0.start.y>=1 && (l0.type==0 && Math.abs(l0.start.x-l0.end.x)<TrackSegment.EPSILON*0.1) || (l0.type==Segment.UNKNOWN && Math.abs(first.start.x-first.end.x)<=E)){
						inTurn = false;
						return;
					} else if (l0.type==0){
						if (first.upper!=null){
					...
					if (first!=null && first.type!=0){
						...
						} else first.leftSeg.center.copy(l0.center);
						if (first.rightSeg.center==null){
							first.rightSeg.center = new Vector2D(r0.center);
						} else first.rightSeg.center.copy(r0.center);
//						first.center.copy(first.leftSeg.center);
		...
		if (highest==null || edgeDetector.highestPoint!=null && highest.y<edgeDetector.highestPoint.y) highest = edgeDetector.highestPoint;
		if (trSz>0){
		...
		for (int i = sL-1;i>=0;--i){
			...
			if (dv>trackWidth){
				if (angle>=0){
					...
					sR = EdgeDetector.join(v, right, sR, trArr, trIndx, trSz, 1);
					sL = CircleDriver2.remove(left, sL, v);					
				}				
			}

			if (dv<=trackWidth-EdgeDetector.EPS){
				isL = 1;
				EdgeDetector.closePointV[EdgeDetector.numClosePoints].copy(v);
				EdgeDetector.closePoint[EdgeDetector.numClosePoints++] = dv<0.1 || angle<=0 ? -1 : 1;
			}
		}
...
//							trSz--;
					...
					sL = EdgeDetector.join(v, left, sL, trArr, trIndx, trSz, -1);
					sR = CircleDriver2.remove(right, sR, v);		
				}				
			}
		...
		if (isL==1 || isR==1){
			...
			int firstGuess = EdgeDetector.closePoint[0];
			int minXIndx = 0;
			double minX = 100;
			int maxXIndx =0;
			double maxX = -100;
			for (int i = EdgeDetector.numClosePoints-1;i>0;--i){
				if (EdgeDetector.closePoint[i]!=firstGuess)
					allSame = false;
				Vector2D v = EdgeDetector.closePointV[i];
				if (v.x<minX){
					minX = v.x;
					minXIndx = i;
				} 
			...
			if (!allSame){
				Vector2D p1 = EdgeDetector.closePointV[minXIndx];
				Vector2D p2 = EdgeDetector.closePointV[maxXIndx];
				if (p1.y>p2.y) 
					firstGuess = 1;
				else firstGuess = -1;
			}
			for (int i = EdgeDetector.numClosePoints-1;i>=0;--i){
				Vector2D v = EdgeDetector.closePointV[i];
				int j = (sL<=0) ? -1 : binarySearchFromTo(left, v, 0, sL-1);
				if (j>=0 && firstGuess==1){					
					...
					j = (sR<=0) ? -1 : binarySearchFromTo(right, v, 0, sR-1);
					if (j>=0){
						for (int k = trSz-1;k>=0;--k){
							int indx = trIndx[k];
							Segment l = trArr[ indx ].rightSeg;
		...
		//		if (l!=null && sL>0)			
			//			Segment.segmentize(left, 0, sL, -1, tW, tr, distRaced,CircleDriver2.l,CircleDriver2.r);
		//				
		//		
		//		if (r!=null && sR>0)			
			//			Segment.segmentize(right, 0, sR, 1, tW, tr, distRaced,CircleDriver2.r,CircleDriver2.l);

		if (inTurn) trSz = Segment.reUpdate(trArr,trSz, tW);		
		//		first = (tr==null || trSz==0) ? null : trArr[0];
	...
	public final void drive(NewCarState cs,CarControl cc) {
		...
		if (debug) System.out.println("**************** E"+time+" "+distRaced+" ****************  "+turn+"      "+edgeDetector.whichE);//
		if (time>=BREAK_TIME)
			System.out.println();
		...
		//		maxSpeed = 60;
		speedControl(cs, maxSpeed,mustPassPoint);
		...
		double slipBF = wheelSpinVel[REAR_LFT] * wheelRadius[REAR_LFT]+ wheelSpinVel[REAR_RGT] * wheelRadius[REAR_RGT]-wheelSpinVel[FRONT_LFT] * wheelRadius[FRONT_LFT]-wheelSpinVel[FRONT_RGT] * wheelRadius[FRONT_RGT];
		slipBF*=0.5;
		if (debug) System.out.println(wheelSpinVel[REAR_LFT]+"    "+wheelSpinVel[REAR_RGT]+"   "+wheelSpinVel[FRONT_LFT]+"   "+wheelSpinVel[FRONT_RGT]+"    "+slip+"     "+slipBF);
		if (debug) System.out.println("Time taken : "+(System.nanoTime()-ti)/1000000);		
	}
	...
	private final void estimateBestPath(){
		...
		if (trSz>1 && lastSeg!=null && lastSeg.type!=0 && speedX>last_speed && (lastSeg.start.y<30 || tooFast && nextSlowSeg.start.y<30 || lastSeg.start.y<40 && speed>200)){			
			...
			double rad = (lastSeg.type==0 || lastSeg.radius>150) ? lastSeg.radius -W*0.5 : (trSz==1 || lastSeg.start.y<=10) ? lastSeg.radius-tW+W*0.5 : lastSeg.radius-tW+W*0.5;
			if (Geom.ptTangentLine(0, 0, lastSeg.center.x, lastSeg.center.y, rad,tmpBuf)>0){
				...
				if (speedRadius>rad+d ||  speedRadius>=dist2Cntr){
					if (mustPassPoint==null) 
						mustPassPoint = new Vector2D( mx,my);
					else {
						mustPassPoint.x = mx;
		...
		if (trSz>1){				
			Segment lastS = (lastSeg.type==1) ? lastSeg.rightSeg : lastSeg.leftSeg;			
			if (!caution && trSz>=2 && lastS.type!=0 && lastSeg.type!=Segment.UNKNOWN && lastS.radius<80){
				...
				double dd = (lastS.radius+0.5*W)/lastS.radius;
				double rry = cny+ ddy*dd;
				if (ry<rry){
					ry = rry;
					rx = cnx+ddx*dd;
			...
			boolean canGo = tryGotoPoint(carDirection, speedRadius, turn, rx, ry, dx, dy,rad, 0, trSz, tmp,true);
			if (canGo){				
				...
				for (int j=trSz-1;j>0;--j){
					...
					if (lastSeg.type!=0 && lastSeg.type!=Segment.UNKNOWN){
						...
						if (dist>=lastS.radius+W){
							Geom.ptTangentLine(0, 0, lastS.center.x, lastS.center.y, lastS.radius+W, tmp);
							directSx = (lastSeg.type==TURNRIGHT) ? tmp[0] : tmp[2];
							directSy = (lastSeg.type==TURNRIGHT) ? tmp[1] : tmp[3];							
						} else {
							directSx = carDirection.x;
							directSy = carDirection.y;
							//							canGoAtCurrentSpeed = false;
						}					
					} else {
						directSx = lastSeg.end.x;
						directSy = lastSeg.end.y;					
						//					draw = true;
					}
					if (isCutLine(0, 0, directSx, directSy, 0, trSz, tmp, W*0.5)!=0) 
						continue;
					else break;
				}
				if (mustPassPoint==null) 
			...
			double rad = (lastS.radius<=150) ? lastS.radius-tW+W*0.5 : lastS.radius-tW+W;
			if (lastS.type!=0 && lastS.type!=Segment.UNKNOWN){
				Vector2D cnt = lastS.center;			
				double cnx = cnt.x;
				double cny = cnt.y;				
				if (Geom.ptTangentLine(0, 0, cnx, cny, rad,tmpBuf)>0){
					double mx = (lastSeg.type==TURNRIGHT) ? tmpBuf[0] : tmpBuf[2];
					double my = (lastSeg.type==TURNRIGHT) ? tmpBuf[1] : tmpBuf[3];									
					//					double d = Math.sqrt(mx*mx+my*my);
					//					double ddx = mx - ox;
	...
	private final double fuzzySteering(CarState cs){
		...
		double angle = Vector2D.angle(carDirection.x, carDirection.y, mustPassPoint.x, mustPassPoint.y);
		if (angle>Math.PI) angle-=Math.PI*2;
		if (angle<-Math.PI) angle+=Math.PI*2;
		double relativePosMovement = (curPos-lastPos)*turn;
		double first_speed = (first==null || first.type==0) ? Double.MAX_VALUE : speedAtRadius(first.radius);
...
//			steer = 0;
		...
		for (i = 0;i<trSz;++i){
			...
			if (t.type!=0){
				...
				double rd = t.radius+W*0.5;
				double d = t.center.length();
				dx = 0;
				dy = 0;			
				if (speedAtRadius(t.radius)<speed-FAST_MARGIN) nextSlowSeg = t;
				if (d>rd){//outside
					...
					if (dy>0){
						dx -= ox;
						dy -= oy;					
						canGoAtCurrentSpeed = (Math.sqrt(dx*dx+dy*dy)>=rad);						
					} else canGoAtCurrentSpeed = false;
				} else canGoAtCurrentSpeed = true;		
				if (i==trSz-1) 
					canGoToLastSeg = canGoAtCurrentSpeed;
				if (canGoAtCurrentSpeed) break;
			}
		}
		...
		if (!canGoAtCurrentSpeed && (absSpeedY<MODERATE_SPEEDY || absSpeedY<HIGH_SPEEDY && absSpeedY<absLastSpeedY) && (toInnerEdge<-W || relativeAngle<-0.001) &&  steer*turn<0 && first.type==turn && speedX>first_speed && (relativePosMovement<=-0.001 || relativeAngleMovement<=-0.001)) 
			steer = -turn;
...
//			steer = -turn;
		...
		double mLen = (trSz>1) ? (!inTurn && lastSeg.type!=0 && lastSeg.type!=Segment.UNKNOWN) ? mLastY : (canGoToLastSeg) ? mLastY : lastSeg.start.y*0.5 : (canGoToLastSeg) ? mLastY : mustPassPoint.y;
		if (mLastY==0 && trSz>1) {
			mLen = Math.max(mLen, lastSeg.start.y);
			canGoToLastSeg = true;
...
//		if ((canGoToLastSeg || canGoA-tCurrentSpeed) && mLen>18 && absSpeedY>=MODERATE_SPEEDY) steer *=0.5;
		if (first!=null && first.type*turn<0 && lastSeg!=null && lastSeg.type!=turn){
			if (toInnerEdge<-tW){
				if (edgeDetector.highestPoint.x*turn>0) 
					steer = gotoPoint(cs, edgeDetector.highestPoint);
//				else steer *= 0.5;
			};
		}
...
//			else dist = Math.sqrt(mLastX*mLastX+mLastY*mLastY);
			if (canGoToLastSeg) 
				maxSpeed = last_speed+FAST_MARGIN+dist;
			else maxSpeed = Math.min(last_speed+FAST_MARGIN+dist,speed -1);
			if (dist>15 && canGoToLastSeg){//keep turning								
					steer = (dist>20) ? (absSpeedY<MODERATE_SPEEDY && relativeTargetAngle>0 && speed>=maxSpeed && relativeAngle>0 && lastSteer*turn>0) ? turn :  steer : steer;				
					return steer;
				...
				if (dist>=20) steer = (relativeAngle>0 && lastSteer*turn>0) ? turn : steer; 
			}
		...
		if (trSz==1 && (relativeSpeedY>0 || relativeAngleMovement<-0.001 && relativeAngle<0.001) && first.type!=0 && absSpeedY<HIGH_SPEEDY && (steer*turn<=0 || Math.abs(steer)<0.01)&& turn!=0 && speedX>first_speed-tW && (relativePosMovement<0.001 || toInnerEdge<-W)){
			...
			//			steer = -turn*s*0.5;
		} else if (trSz==1 && (steer*turn>0 || Math.abs(steer)<0.001) && first.type!=0 && first.type*turn<0 && relativeSpeedY<-0.001){
			steer = (lastSteer*steer<0) ? (Math.abs(lastSteer)>s) ? 0 : lastSteer : 0; 
		} else if (trSz==1 && speedX>lowestSpeed+20 && first.type*turn>0 && toInnerEdge<-W*0.5 && steer*turn>0 && (relativePosMovement<-0.001 || relativeAngleMovement<-0.001))
		...
		if (canGoToLastSeg && mLastY>0&& (trSz==1 && first.type!=0 || lastSeg.type!=0) && mLen>=15 && relativeAngle>0 && relativePosMovement>0 && relativeAngleMovement>0){
			if (relativeTargetAngle>0)
				steer *= 0.5;
			else steer *=1.5;
		}

//		double d = !inTurn && (canGoAtCurrentSpeed || canGoToLastSeg) ? mLen : (canGoAtCurrentSpeed || canGoToLastSeg || maxTurn )&& absSpeedY<=HIGH_SPEEDY ? mLen : 
...
//				(trSz==1 || toOutterEdge<SAFE_EDGE_MARGIN) && speed>first_speed+10 && relativeAngle<-0.001 && (relativePosMovement<-0.001 || relativeAngleMovement<-0.001)  || !inTurn && (nextSlowSeg==null || lowestSeg==null || nextSlowSeg.radius>lowestSeg.radius || nextSlowSeg.start.y<20) ? 0 : mLen;
		double d = mLen;
		if (Math.abs(lowestSpeed-last_speed)<FAST_MARGIN) targetRadius = lastSeg.radius;
		double targetSpeed = (canGoAtCurrentSpeed || canGoToLastSeg) ? speedAtRadius(targetRadius) : lowestSpeed;
		if (targetSpeed>highestSpeed) targetSpeed = highestSpeed;
//		double ofsDist = mLen>18 || Math.abs(relativeTargetAngle-relativeAngle)>0.1 && relativeAngle>=0 && curPos*turn>0.3 ? FAST_MARGIN : tW*3;
		double ofsDist = FAST_MARGIN;

		if (trSz>1 && inTurn && absSpeedY<MODERATE_SPEEDY && last_speed==highestSpeed && relativePosMovement>0 && relativeAngle>0){
			...
			maxSpeed = (trSz>1 && (speed>highestSpeed && lowestSeg.start.y<15 || inTurn && highestSpeed-lowestSpeed<15 &&  relativePosMovement<-0.001)) ? lowestSpeed : Math.max(targetSpeed,lowestSpeed+ofsDist)+d;
		} else maxSpeed = (!canGoAtCurrentSpeed || absSpeedY>=VERY_HIGH_SPEEDY && relativePosMovement<-0.001) ? targetSpeed+d : (canGoAtCurrentSpeed || canGoToLastSeg) && absSpeedY<=MODERATE_SPEEDY && (relativePosMovement>-0.001 || relativeAngleMovement>-0.001 && relativeAngle>0.001) ?  targetSpeed+d+ofsDist : 
			(trSz==1 || highestSpeed-lowestSpeed>15 || canGoToLastSeg) && ((relativePosMovement>=0 || relativeAngleMovement>=0 && canGoToLastSeg) && relativeAngle>0.001) ? targetSpeed+d+ofsDist : targetSpeed+d;
...
//					if (maxSpeed>targetSpeed && highestPoint!=null && highestPoint.y<60) maxSpeed = targetSpeed;
					...
					if (d>18 && absSpeedY<=MODERATE_SPEEDY){						
						...
						if (canGoAtCurrentSpeed) maxSpeed += d;
	...
	private final void record(CarState cs){		
		...
		for (int i = 0;i<trSz;++i){
			...
			if (lSeg.start.y>prevEdge.left[lSeg.startIndex].y+SMALL_MARGIN || lSeg.end.y<prevEdge.left[lSeg.endIndex].y-SMALL_MARGIN)
				System.out.println();
		...
		for (int i = 0;i<trSz;++i){
			...
			if (rSeg.start.y>prevEdge.right[rSeg.startIndex].y+SMALL_MARGIN || rSeg.end.y<prevEdge.right[rSeg.endIndex].y-SMALL_MARGIN)
				System.out.println();
			if (i>0){
		...
		if (inTurn || (first!=null && (first.type!=0 || Math.abs(first.end.x-first.start.x)>E))){
			//			l = (left==null) ? null : (inTurn) ? Segment.segmentize(left, tW) : l;
		...
		//		
		if (!inTurn && (first.type==0 && first.end.y-nraced<1 || !(prevEdge.straightDist<=0  || (nraced<=0 && first!=null && first.type==0 && first.start.y<nraced-1) || nraced<prevEdge.straightDist-0.3 || prevEdge.center!=null && prevEdge.center.y>=nraced))){
			lMap.setRowLen(sL);
			rMap.setRowLen(sR);
			combine(first, left, sL, right,sR);
			sL = edgeDetector.lSize;
		...
		if (trSz>1 && lastSeg.type!=0 && lastSeg.type==turn && toOutterEdge<=W*0.5 && toInnerEdge>=-W*0.5 && (lastSeg.leftSeg.num<2 || lastSeg.rightSeg.num<2)){
			double rad = lastSeg.radius-tW+W*0.5; 
			if (Geom.ptTangentLine(0, 0, lastSeg.center.x, lastSeg.center.y, rad,tmpBuf)>0){
				double mx = (lastSeg.type==TURNRIGHT) ? tmpBuf[0] : tmpBuf[2];
				//double my = (lastSeg.type==TURNRIGHT) ? tmpBuf[1] : tmpBuf[3];
				if (mx*turn<0) 
		...
		if (inTurn && first!=null && first.type==0 && Math.abs(first.start.x-first.end.x)<EPSILON) inTurn = false;
		if (inTurn && (l!=null || r!=null)){			
			if ((l!=null && l0!=null && l0.type==0 && l0.end.y>=1 && Math.abs(l0.end.x-l0.start.x)<TrackSegment.EPSILON) 
					|| (r!=null && r0!=null && r0.type==0 && r0.end.y>=1 && Math.abs(r0.end.x-r0.start.x)<TrackSegment.EPSILON)) {
				inTurn = false;
		...
		toInnerEdge = curPos*turn*tW - tW -W*0.5;
	...
	private final double steerControl(CarState cs){											
		...
		if (curPos>1 || curPos<-1){
			...
			double steer = (absSpeedY>=HIGH_SPEEDY) ? turn-deflect : (first!=null && first.type*curPos>0 && speed>speedAtRadius(first.radius)) ? (relativeAngle>=-0.001 || relativeAngleMovement>=0.001) ? ((relativeSpeedY<-0.001 || absSpeedY<=LOW_SPEEDY)&& trSz>1 && trArr[ trIndx[trSz-1] ].type==0) ? curAngle/steerLock : -deflect : -turn-deflect : (curAngle*turn>0) ? 0 : curAngle/steerLock; 
			return steer+deflect;
		}
		if (trackWidth<=0)
		...
		if (turn==UNKNOWN){
			...
			return (curAngle)/steerLock+sign*0.1;
		}

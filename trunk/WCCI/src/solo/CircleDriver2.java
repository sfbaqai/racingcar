	...
	private double BRAKE_HARD(double a,double b,double m,double lastSpeed,double absSpeedY,double targetSpeed){
		return relativePosMovement<-0.001 && relativeAngleMovement<-0.01 && steer*turn<0 ? brake*0.5 
				...
				: (relativeAngleMovement>0.01 && distToEstCircle>0) ? 1 : brake 
					: (relativeAngleMovement<-0.005 && distToEstCircle<GAP*0.5|| absSpeedY>=MODERATE_SPEEDY && m<=20 && distToEstCircle<0 || distToEstCircle<0 || (m<=20 || !isSafeToAccel) && (canGoModerate || canGoVeryFast)) ? brake : 1;
...
//					: 1;
	...
	private double safeDist(double al,double hl){
		if (relativeTargetAngle<0) return Math.min(100, hl);  
		return Math.min(100, al);
	}
...
//			targetSpeed = lowestSpeed+FAST_MARGIN+mLen;
		...
		double mBrk = (speed>sp) ? Math.min(brake, (speed*speed-sp*sp)*0.5/(speed*speed)) : 0;
			...
			if (speedX>targetSpeed+FAST_MARGIN){
				...
				steer = relativeAngleMovement>0.02 || relativeAngle>0.3 && relativeAngleMovement>0.001 ? turn 
						: (relativeAngleMovement>0.01) 
							...
							: (relativeAngleMovement>0.001 && relativeAngleMovement>lastRelativeAngleMovement) ? (relativeAngle>0.2) ? 0 : steer 
...
//				acc = 0;
				acc = (speed>targetSpeed || relativeAngleMovement<-0.001 || relativeAngleMovement>-0.001 && relativeAngle>0.3 || relativeAngle>0.25 && relativeAngleMovement>0.01) ? CONSTANT_SPEED_ACC*0.25 : 1;
				brake = 0;
					...
					steer = (relativeAngleMovement>0.02  || relativeAngle>0.3 && relativeAngleMovement>0.001 || relativeAngleMovement>0.015 && relativeAngleMovement>lastRelativeAngleMovement) 
							...
							: relativeAngleMovement>0.01 && relativeAngleMovement>lastRelativeAngleMovement 
								...
								: relativePosMovement<-0.01 && relativeAngleMovement>lastRelativeAngleMovement && relativeAngleMovement>0.001 || relativeAngleMovement>0.01 
									? (relativeAngle>0.25) ? bal : (relativeAngle>0.2) ? 0 : steer 
...
//				acc = CONSTANT_SPEED_ACC;
			...
			if (brake==0 && acc==0) acc = (speedX<targetSpeed || speedX<targetSpeed+10 && relativeAngleMovement>-0.001) ? CONSTANT_SPEED_ACC : CONSTANT_SPEED_ACC*0.25;
//			steer =0;
			return steer;
		}
		...
		if (flyingHazard || speedX>50 && speedX<lastSpeedX && slip>10 && slip>0 && lastSlip>0 && balance>0){
			...
			if ((flyingHazard && !landed || startFlying>0 && relativeAngle>0 && relativeAngleMovement>0.001 && steer*tpe<0 && speedX<lowestSpeed) && absSpeedY<MODERATE_SPEEDY && relativeTargetAngle>0)
				...
				steer = (relativeAngle<0) 
							...
							: relativeAngleMovement>0.02 ? tpe : relativeAngleMovement>0.01 && steer*tpe<0 ? stableSteer(0) : steer;
			else if (a>0){
...
//			if (relativePosMovement<0 || absSpeedY>MODERATE_SPEEDY) smoothSteering();
			acc = (!mustSlowDown && slip>15 && Math.abs(a)<0.5 && absSpeedY<HIGH_SPEEDY) ? 1 
					: (!maxTurn && relativeAngle<0 || Math.abs(a)>0.5) ? CONSTANT_SPEED_ACC*0.25 : !flyingHazard && startFlying==0 && mustSlowDown || speedX>targetSpeed+10 || relativePosMovement<-0.02 && absSpeedY>MODERATE_SPEEDY && relativeAngleMovement<-0.001 || speed>targetSpeed-tW && absSpeedY>45
					|| speed>sp && speed>highestSpeed && speed>targetSpeed && relativePosMovement>0.001 && distToEstCircle<0 && relativeAngleMovement<-0.001 && relativeAngleMovement<lastRelativeAngleMovement
					? 0.25*CONSTANT_SPEED_ACC: 1;
					...
					steer = (speedX<lowestSpeed || distToEstCircle>GAP) ? (steer+lastSteer)*0.5 
							: (distToEstCircle<-GAP)
								? (speedX<lastSpeed-15) ? steering 
										: (turn*steering>0) ? steering : -turn 
//											maxAbs(-turn*relativeTargetAngle*0.5/steerLock,steering)
...
//					smoothSteering();
				...
				smoothSteering();
				/*if (a<0 && absSpeedY>MODERATE_SPEEDY && relativePosMovement>0.01 && relativeAngleMovement>0.01){
					acc = 0;
//					if (brake>sp) brake = mBrk;
					brake = 0;
				} else//*/ 
...
//			if (a>0 && (relativePosMovement>0.02 || relativePosMovement>0.015 && relativePosMovement>lastRelativePosMovement) && steer*turn<0) steer *= 0.5;
...
//			if (distToEstCircle<-W && relativePosMovement>0.001 && relativeAngleMovement<-0.02) acc = Math.min(acc,CONSTANT_SPEED_ACC);
...
//						acc = (speedX>first_speed && alpha>=PI_2*0.5 || dangerSlip) ? 0 : acc*CONSTANT_SPEED_ACC;
					...
					acc = mustSlowDown ? 0 :(!maxTurn || (dangerSlip || absSpeedY>10) && relativeAngle<0) 
							...
							: (maxTurn && (absSpeedY<MODERATE_SPEEDY && relativeAngle>0 && toOutterEdge>=W*0.75 
								...
								|| (edgeDetector.highestPoint==null || hl-m<50) && relativePosMovement>-0.01 && relativeAngle>-0.15 
...
//											: 0
									? (maxTurn) 
											? mustSlowDown && !isSafeToAccel && speedX>highestSpeed ? 0 : (isSafeToAccel && speedX<first_speed && !dangerSlip && (distToEstCircle>=-W || relativeAngleMovement>0.01 && distToEstCircle>-W*1.5) 
													...
													: acc*CONSTANT_SPEED_ACC*0.25 
											: 0
										: (relativeAngle<-0.001 && (relativeAngleMovement<-0.001 || dangerSlip && distToEstCircle<-tW || dangerSlip && slip>15) && speedX>lowestSpeed-tW*2 && !isSafeToAccel) 
											? 0 
...
//			acc *= INCREASE_ONE;
		...
		if (turn!=0 && turn!=2 && acc>CONSTANT_SPEED_ACC && absSpeedY>MODERATE_SPEEDY && speedX>targetSpeed-10 && speedX>lowestSpeed && relativeAngleMovement<-0.01 && (relativePosMovement<0.001 || distToEstCircle<0)) 
			acc = CONSTANT_SPEED_ACC*0.25;
		
		if (turn!=0 && turn!=2 && a>0 && acc>CONSTANT_SPEED_ACC && speedX>targetSpeed-10 && speedX>lowestSpeed && relativeAngleMovement<lastRelativeAngleMovement && relativePosMovement<-0.01 && relativePosMovement<lastRelativePosMovement && distToEstCircle<0) 
			acc *= CONSTANT_SPEED_ACC;
		
		if (acc>=CONSTANT_SPEED_ACC && relativePosMovement<0.001 && distToEstCircle<-tW && absSpeedY>=MODERATE_SPEEDY && speed>sp)
			acc = 0;
...
//									brake = 1;
								...
								brake = (distToEstCircle>0 && relativeAngleMovement>0.01 && absSpeedY<MODERATE_SPEEDY || isSafeToAccel && steer*turn>=0 && distToEstCircle>GAP) ? 1 : (isSafeToAccel && steer*turn<0 || absSpeedY>=MODERATE_SPEEDY || relativeAngleMovement<-0.01) ? brake : 1;								
...
//							brake = (speed*speed-targetSpeed*targetSpeed)/(speed*speed);
							if (isSafeToAccel && mLen>15 && toInnerEdge<-W*1.5){
								...
								brake = (relativePosMovement>0 && relativeAngleMovement>0.001) ? 1 : (isSafeToAccel && steer*turn<0 || absSpeedY>=MODERATE_SPEEDY) ? brake : 1;
									...
									? lastSpeed>90 && (highest==null || highest.y>30) && (seenNewSeg && highest!=null && highest.y>50 || speedX<targetSpeed+15 && lastSpeed>120 || canGoVeryFast || canGoModerate) 
										? 0 
										: brake 
									: steer*turn<0 ? brake : 0.5*brake;
...
//							steer = (steer*turn<0) ? -turn : (steer==0) ? 0 : turn;
							...
							brake = (relativeAngleMovement<0 && distToEstCircle<-GAP) ? brake*0.5 : brake;							
						} else if (relativePosMovement<0) 
							...
							? isSafeToAccel || relativePosMovement<-0.02 && relativeAngleMovement<-0.001 || absSpeedY>HIGH_SPEEDY && (relativeAngleMovement>0.01 || relativeAngleMovement>0 && relativeAngleMovement>lastRelativeAngleMovement) 
										|| relativePosMovement<-0.001 && (relativeAngleMovement<-0.005 || relativeAngleMovement<-0.001 && absSpeedY>MODERATE_SPEEDY) 
											? (speed>sp) ? Math.min(brake, (speed*speed-sp*sp)*0.5/(speed*speed)) : 0 
											: (absSpeedY>HIGH_SPEEDY && speed>sp) ? Math.min(brake, (speed*speed-sp*sp)*0.5/(speed*speed)) : 0 
							: brake;
							...
							brake = (isSafeToAccel && steer*turn<0 || absSpeedY>=MODERATE_SPEEDY || distToEstCircle<-W) ? brake : 1;
								...
								? (distToEstCircle<-W*1.2 && !isSafeToAccel) 
										...
										: relativeAngleMovement>-0.01 && (seenNewSeg && (absSpeedY<MODERATE_SPEEDY || absSpeedY<absLastSpeedY) || speedX<targetSpeed+15 && distToEstCircle>lastDistToEstCircle && absSpeedY<MODERATE_SPEEDY) 
											? (seenNewSeg && highest!=null && highest.y<50 && distToEstCircle<-GAP*0.5 && distToEstCircle<lastDistToEstCircle && relativePosMovement>-0.001 || distToEstCircle<-GAP) ? brake : 0 
											: brake 
...
//						(mLastY>0 && speed>=targetSpeed+Math.min(mLen,5)) ? brake : 0;
					...
					if ((absSpeedY>HIGH_SPEEDY || absSpeedY>=MODERATE_SPEEDY && relativeAngleMovement<0 && relativePosMovement<0 && relativeTargetAngle>0) && speed>targetSpeed+FAST_GAP || mLastY>0 && speed>=targetSpeed+FAST_GAP && !mustTurn || speed>=targetSpeed+FAST_GAP && relativeSpeedY<0 && !canGoToLastSeg){ 
						if (isSafeToAccel && toInnerEdge<-W*1.5 || (absSpeedY<=MODERATE_SPEEDY || relativePosMovement>0) && canGoAtCurrentSpeed && canGoToLastSeg) 
							brake = !canGoModerate && ! canGoVeryFast && (a<0 || a>0 && distToEstCircle>-GAP && (relativeAngleMovement>0.01 || distToEstCircle>0 && relativeAngleMovement>-0.01 ||  m>15 && distToEstCircle>GAP*0.5 && speedX>targetSpeed+FAST_MARGIN)) 
								? 1
								: brake;						
					} else if (speed>targetSpeed+MODERATE_GAP){ 
						brake = (speed*speed-targetSpeed*targetSpeed)/(speed*speed);
						if (mustTurn && relativePosMovement>=0 && !canGoToLastSeg) {
							...
							brake = !canGoModerate && ! canGoVeryFast && (absSpeedY<MODERATE_SPEEDY || speed>targetSpeed+25) ? 1 : brake;
//							steer = 0;
						}
					} else if (speed>targetSpeed+Math.max(10,mLen*0.5)){ 
						brake = speed>=targetSpeed+10 && relativePosMovement<-0.001 || targetSpeed<=100 || absSpeedY>=MODERATE_SPEEDY && speed>targetSpeed+5 ? brake*0.5 : inTurn && (canGoVeryFast || canGoModerate) ? 0 : brake;
//						if (absSpeedY>MODERATE_SPEEDY) 
//							brake *= (m<20) ? 1.5 : 0.75;
//						if (!isSafeToAccel && speedX>=targetSpeed+15) brake = Math.min(brake,0.1);	
					...
					} else if (speed<targetSpeed+10 && absSpeedY<MODERATE_SPEEDY) brake = 0;
...
//				}
				steer = distToEstCircle>0 && (relativeTargetAngle<-0.001 && relativePosMovement>0.005 ||  relativePosMovement>0.01) 
					? (relativeAngleMovement>0) 
							? (a>0.05)  
									...
									: (steer*bal>0) ? minAbs(steer,bal) : 0 
				...
				steer = !possibleChangeDirection && canGoToLastSeg && absSpeedY<10 && m>10 || relativeAngleMovement>0.02 
						|| relativeAngleMovement>0.01 && relativeAngleMovement>lastRelativeAngleMovement && distToEstCircle>-W && (a<TURNANGLE*0.5 || distToEstCircle>0)? 0 : steer;
...
//				steer = 0;
			else if (isSafeToAccel && !canGoVeryFast && distToEstCircle>0 && distToEstCircle<W && b>TURNANGLE*0.75 && (speedX>targetSpeed+25 && m>30 || speedX>targetSpeed+5 && m>40 || speedX>targetSpeed && m>45 || brake==1 && m>15) && (relativeAngleMovement<0.01 || distToEstCircle<lastDistToEstCircle)){
				steer = -turn;			
...
//			}
				...
				if (distToEstCircle<0 && steer*turn<0){
				 ...
				 if (relativeAngleMovement>-0.005 || relativeAngleMovement>-0.01 && lastRelativeAngleMovement<relativeAngleMovement) 
					...
					brake = (speedX>targetSpeed+30 || steer*turn>0) ? 1 : (speed>sp) ? Math.min((speed*speed-sp*sp)/(speed*speed),brake) : brake;
  			 } 
...
//				hazard = 2;
			...
			if (canGoVeryFast && speedX-lastSpeed<dl*1.5) {				
				if (a>0 && b>0 && (al>50 || distToEstCircle>-W && absSpeedY<MODERATE_SPEEDY || distToEstCircle>-GAP && absSpeedY<HIGH_SPEEDY || (distToEstCircle>-W || relativeAngleMovement>0.01) && b>TURNANGLE*0.5 && relativePosMovement>-0.001 && absSpeedY<HIGH_SPEEDY)) {
					acc = (distToEstCircle<0 && (relativePosMovement<0.01 && speedX>lastSpeed+dl || relativeAngleMovement<-0.01) ) ? acc : (relativeAngleMovement>-0.001 || absSpeedY<MODERATE_SPEEDY && distToEstCircle>0) ? 1 : acc;
					if (acc>0 || relativeAngleMovement>-0.01 && (speedX<aheadSpeed+dl || speedX<aheadSpeed+dl*1.5 && distToEstCircle>-GAP*0.5)) brake = 0;
				} else if (a>0 && b>0 && absSpeedY<HIGH_SPEEDY && relativePosMovement>-0.001 && relativeAngleMovement>0.001) brake = 0;
				if (acc<=CONSTANT_SPEED_ACC*0.25 && brake==0 && relativeAngleMovement>0.001 && speedX<lowestSpeed || acc>CONSTANT_SPEED_ACC && relativeAngleMovement<-0.01 && distToEstCircle<-GAP && relativePosMovement<-0.001) acc = CONSTANT_SPEED_ACC;				
...
//						acc = 1;
						brake = 0;						
					} else if (relativePosMovement>-0.001 && speed>sp+10){ 
						brake  =0;
...
//				acc = CONSTANT_SPEED_ACC;
			/*else if (acc<1 && a>0 && b>0 && speedX<lastSpeed+al && (relativePosMovement>-0.001 && distToEstCircle>-W || distToEstCircle>-GAP)){
				acc = (relativeAngle<-0.001 && relativeAngleMovement<-0.001) ? acc : 1;
				brake = 0;
			}//*/
			
//			if (acc>CONSTANT_SPEED_ACC && relativeSpeedY<0 && absSpeedY>MODERATE_SPEEDY && relativePosMovement>-0.001 && m<15 && (a<0 || distToEstCircle>0 && relativePosMovement>0.01 && absSpeedY>absLastSpeedY))
//				acc = CONSTANT_SPEED_ACC;
			
			if (a>0 && b>0 && !canGoModerate && brake>0 && absSpeedY<MODERATE_SPEEDY && (speed<Math.min(lastSpeed,aheadSpeed)+dl+10 && distToEstCircle<-GAP*0.5 || distToEstCircle>-GAP*0.5 && speed<Math.min(lastSpeed,aheadSpeed)+dl*1.5+10 && !isSafeToAccel) && (relativePosMovement>-0.001 && distToEstCircle>-W || distToEstCircle>-GAP)){				
				if ((speed<sp+10 && distToEstCircle<0 || distToEstCircle>0 && speed<Math.min(lastSpeed,aheadSpeed)+dl*1.5+10 && !isSafeToAccel)) 
//					brake = 0;
					brake = (distToEstCircle<-GAP && relativeAngleMovement<-0.001 || distToEstCircle<-W || distToEstCircle<0 && relativeAngleMovement<-0.01) 
						?  speed>sp ? Math.min(brake,(speed*speed-sp*sp)*0.5/(speed*speed))*0.5 : 0
						: (speed>sp && relativeAngleMovement<-0.001) ? Math.min(brake, (speed*speed-sp*sp)/(speed*speed)) : 0;
				else brake = (speed<sp) ? 0 : (absSpeedY<MODERATE_SPEEDY) ? Math.min(brake,(speed*speed-sp*sp)*0.5/(speed*speed)) : Math.min(brake,(speed*speed-targetSpeed*targetSpeed)*0.5/(speed*speed));
			} 
			
			if (a>0 && b>0 && brake>0 && absSpeedY>20 && absSpeedY>absLastSpeedY) {
				
				brake = (absSpeedY>HIGH_SPEEDY || relativePosMovement<-0.001) 
					? (speed>sp) ? Math.min(brake,(speed*speed-sp*sp)*0.5/(speed*speed)) : brake*0.5
					: brake;
//					:relativeAngleMovement>0.001 ? brake*0.5 : brake;
			}//*/
			if (a>0 && b>0 && brake>0 && absSpeedY>=MODERATE_SPEEDY && relativePosMovement>0.001 && speed<Math.max(sp,targetSpeed)+10){
				brake =  (speed<sp) ? (distToEstCircle<0 || relativeAngleMovement<-0.01 || absSpeedY>20 && absSpeedY>absLastSpeedY) ? brake : 0 : speed>sp && relativeAngleMovement>-0.01 ? Math.min(brake,(speed*speed-sp*sp)*0.5/(speed*speed)) 
						: distToEstCircle>0 || relativePosMovement>0.01 && distToEstCircle>-GAP ? 0 : speed>sp ? Math.min(brake,(speed*speed-sp*sp)/(speed*speed)): brake;
...
//			if (speed-lastSpeed<Math.min(100, dl*1.5) && b>TURNANGLE && m<10 && a>0 && relativePosMovement>0.02 && brake>0 && relativeAngleMovement>0.001) brake = 0;
			
//			if (a>TURNANGLE && b>TURNANGLE && acc>CONSTANT_SPEED_ACC && turn!=2 && (highest.x - ahead.x)*turn>0 && relativePosMovement<-0.015 && relativeAngleMovement<-0.01)
//				acc = CONSTANT_SPEED_ACC;
//			else if (m<10 && b>TURNANGLE && acc>CONSTANT_SPEED_ACC && turn!=2 && steer*turn<0 && speed>lastSpeed && relativePosMovement>0.01 && relativeAngleMovement<-0.01)
//				acc = CONSTANT_SPEED_ACC;
			
			
			
//			if (a<0 && relativePosMovement>0.01 && relativeSpeedY<0 && acc>CONSTANT_SPEED_ACC) 
//				acc = (speedX<lowestSpeed) ? acc : m>25 ? CONSTANT_SPEED_ACC : 0;
			
			
//			else if (a>0 && b>0 && !canGoModerate && brake>0 && absSpeedY<MODERATE_SPEEDY && relativeAngleMovement<-0.001 && (speed<Math.min(lastSpeed,aheadSpeed)+dl+10 && distToEstCircle<0 && distToEstCircle>-GAP*1.5 || distToEstCircle>0 && speed<Math.min(lastSpeed,aheadSpeed)+dl*1.5+10 && !isSafeToAccel) && relativeAngleMovement>-0.01){
//				double sp = distToEstCircle<0 ? Math.min(lastSpeed,aheadSpeed)+dl : Math.min(lastSpeed,aheadSpeed)+dl*1.5;
//				if (sp<targetSpeed) sp = targetSpeed;
//				brake = Math.min(brake, (speed*speed-sp*sp)/(speed*speed));
//				brake =0;
//			}
...
//				acc = CONSTANT_SPEED_ACC;
			...
			if ((relativeAngle>0.3 && relativeAngleMovement>0 || relativeAngle>0.35 || relativeAngle>0.25 && relativeAngleMovement>0.01) && relativePosMovement<-0.001 || relativeAngle>0.5)
				steer = (relativeAngleMovement<0 && a>0) 
							? (steer*turn<=0) ? steer : 0 
							: (a>0 && relativeAngleMovement<0.01 && relativeAngleMovement<lastRelativeAngleMovement) 
								? 0 
								: (relativeAngleMovement<0.015 && relativeAngleMovement<lastRelativeAngleMovement) ? 0 : turn;
	...
	private final void combine(Segment first,Vector2D[] left, int sL,Vector2D[] right,int sR){
		...
		if (first==null){			
			...
			if (tp==0 && Math.abs(first.end.x-first.start.x)>TrackSegment.EPSILON*0.1){
				int tpe = (firstRad==0) ? 0 : first.end.x-first.start.x>0 ? 1 : -1;
...
//				} else whichH = whichL = (highestPoint.x-ph.x)*turn<0 ? -turn : 0;
		...
		first = (trSz==0) ? null : trArr[trIndx[0] ];
		if (first!=null && first.type==0 && Math.abs(first.start.x-first.end.x)<=E) inTurn = false;			
	}
...
//				trackData.add(ts);
	...
	private double danger(double a,double b,double absSpeedY,double steer,double lSteer){
		if (distToEstCircle<-GAP && b>TURNANGLE*0.5 || b>TURNANGLE*0.75 && distToEstCircle<0)
			return distToEstCircle>=0 || b<TURNANGLE/1.5 ? (b<TURNANGLE*0.5 && steer*turn>0) ? steer : 0 : (distToEstCircle>=-GAP && absSpeedY<10) 
			...
			return (distToEstCircle<0) ? lSteer : (a>TURNANGLE/1.5 && absSpeedY<MODERATE_SPEEDY) ? (b>TURNANGLE*0.75 && distToEstCircle<GAP) 
						...
						: steer*0.5 
					: (b>TURNANGLE || a>TURNANGLE*0.5) && distToEstCircle<GAP ? distToEstCircle>0 && distToEstCircle>lastDistToEstCircle ? lSteer*0.5 : lSteer 
							: (distToEstCircle>GAP) ? ((b<TURNANGLE*0.5 || relativePosMovement>0.01 && absSpeedY>25) && steer*turn>0) ? steer : 0 : steer;
...
//		hazard = 2;
		if (steer*turn<0 || Math.abs(steer)<0.001) return (relativeAngleMovement<-0.01) 
				? -turn 
...
//			hazard = 2;
			...
			hazard = 2;
			return (distToEstCircle>W && relativeAngleMovement>-0.01) ? (steer+lastSteer)*0.5 : 0;
		} else if (steer*turn>=0){			
			hazard = 2;
			return (distToEstCircle<W && b>TURNANGLE*0.5 || a>b && distToEstCircle<W) ? (distToEstCircle<0) ? lSteer*0.5 : 0 : (relativeAngleMovement<-0.01) ? 0 : (steer+lastSteer)*0.5;
		}
		return steer*turn<0 && lastSteer*turn<0 && (b>TURNANGLE || a>TURNANGLE*0.5 && distToEstCircle<GAP) ? maxAbs(steer,lastSteer) : steer;
	}
...
//						else steer = turn;p
						...
						if (isDanger){
							...
							steer = (m<30 && distToEstCircle<-GAP && a>TURNANGLE/1.5 || b>TURNANGLE && distToEstCircle<-GAP || speedX>maxSpeed+10 && distToEstCircle<0) 
								...
								: distToEstCircle<GAP && (relativeAngleMovement<-0.01 || b>TURNANGLE || a>TURNANGLE*0.5 || distToEstCircle<0 && (relativeAngleMovement>0.01 || speedX>maxSpeed+10)) ? lSteer*0.5 : steer;
...
//							if (speed>maxSpeed && steer*turn>0) steer=0;
					...
					if (steer*turn<0 && a>0 && distToEstCircle>-W && distToEstCircle>lastDistToEstCircle)
						...
						steer = distToEstCircle<-GAP && (relativePosMovement<-0.01 || relativeAngleMovement<-0.01 || speedX>lowestSpeed) ? -turn : steer;
...
//					} else 
					if (b>TURNANGLE && speedX>maxSpeed && a>0 && steer*turn>0)
						...
						steer = (speedX<lowestSpeed-15 && a>0 && distToEstCircle>-tW) ? minAbs(steer,lSteer*0.5) : (m>30 && speedX<MODERATE(lowestSpeed, m) && a<TURNANGLE*0.5) ? relativeAngle>0 ? 0  : maxAbs(steer,lSteer*0.5) : 
					...
					if (steer*turn<=0 && (distToEstCircle<-GAP || relativeAngle<0 && lastDistToEstCircle>distToEstCircle && distToEstCircle<0) && (isDanger || absSpeedY>=MODERATE_SPEEDY)) 
						steer = (isOffBalance && distToEstCircle<-GAP || relativeSpeedY<0) 
							? (relativeAngleMovement>0.01) ? 0 : maxAbs(steer,lSteer*0.5) 
							: (distToEstCircle>-GAP || lastDistToEstCircle<distToEstCircle && distToEstCircle>-W) ? steer : maxAbs(steer,lSteer);
					
					if (relativeSpeedY<0 && steer*turn<0 && speedX>maxSpeed) steer = minAbs(steer,lSteer)*0.5;
					if ((relativeAngleMovement<-0.001 && relativePosMovement<-0.01 || relativeAngleMovement<-0.01 && relativePosMovement<0) && speedX>first_speed && !flyingHazard && slip<20) steer = -turn;
					if (speedX<maxSpeed && distToEstCircle>0 && relativeAngleMovement>0.01){
						steer = relativeSpeedY>0 && steer*turn>=0 && relativeAngleMovement>lastRelativeAngleMovement && b<TURNANGLE*0.5 ? turn : steer;
//						hazard = 2;
...
//								steer = (absSpeedY>MODERATE_SPEEDY) ? lSteer : (absSpeedY>10) ? lSteer*0.5 : 0;
						...
						if (isOffBalance && a>0)
							steer = (m<30 && distToEstCircle<-GAP && a>TURNANGLE/1.5 || b>TURNANGLE && distToEstCircle<-GAP || speedX>maxSpeed+10 && distToEstCircle<0) 
								? (Math.abs(lSteer)>Math.abs(steer)) ? (lSteer+steer)*0.5 : 0 
								: (relativeAngleMovement<-0.01 || b>TURNANGLE || a>TURNANGLE*0.5 || distToEstCircle<0 && (relativeAngleMovement>0.01 || speedX>maxSpeed+10)) ? lSteer*0.5 : steer*0.85;
...
//						hazard = 2;
				...
				if (m>20 && canGoToLastSeg && c<TURNANGLE*2 || m>45 || m>25 && distToEstCircle>-tW){
//					double cx = lastSeg.center.x;
//					double cy = lastSeg.center.y;
						...
						if (absSpeedY>=MODERATE_SPEEDY || distToEstCircle<-W || c>TURNANGLE) 
							steer = (relativeAngle<0 && isFast && m<25) ? (steer+lastSteer)*0.5 
								...
								? speedX>faster(last_speed,m)+10 && toInnerEdge<-GAP && b>TURNANGLE*0.75 && a>0 && m>10 || distToEstCircle<-GAP && b>TURNANGLE/1.5 || distToEstCircle<0 && isDanger 
										? (steer*lSteer>0) ? maxAbs(steer, lSteer) : (distToEstCircle>-GAP) ? lSteer : -turn 
										: (distToEstCircle>0) 
											? (relativeAngleMovement>0.015 || relativeAngleMovement>0.01 && relativeAngleMovement>lastRelativeAngleMovement) 
													? (relativeSpeedY<0 && relativeAngleMovement>lastRelativeAngleMovement) ? steer*turn>0 ? steer : bal*turn>0 ? bal*0.5 : 0 : turn 
													: steer*turn>0 && (a<0 || b<TURNANGLE*0.75) ? steer : 0
											: (distToEstCircle>-GAP) ? 0 : lSteer
								: (m<25 && speedX>last_speed+Math.max(FAST_MARGIN+tW,m)) 
									? 0 
									: (b>TURNANGLE/1.5) 
										? (distToEstCircle<0) 
												? (speedX<lowestSpeed && distToEstCircle>-tW) ? lSteer*0.5 : (distToEstCircle>-GAP) ? lSteer : -turn 
												: (steer+lSteer)*0.5 
										: (speedX<lowestSpeed) 
											? 0  
											: steer;
...
//						else steer = turn;p
						...
						if (isOffBalance && a>0)
							steer = (m<30 && distToEstCircle<-GAP && a>TURNANGLE/1.5 || b>TURNANGLE && distToEstCircle<-GAP || speedX>maxSpeed+10 && distToEstCircle<0) 
								? (Math.abs(lSteer)>Math.abs(steer)) ? (lSteer+steer)*0.5 : 0 
								: (relativeAngleMovement<-0.01 || b>TURNANGLE || a>TURNANGLE*0.5 || distToEstCircle<0 && (relativeAngleMovement>0.01 || speedX>maxSpeed+10)) ? lSteer*0.5 : steer;
...
//			steer = (-turn*relativeTargetAngle/steerLock+ gotoPoint(cs, 0, 1))*0.5;
		...
		//			steer = -turn;
		if (!flyingHazard){
			if (relativePosMovement>0 && distToEstCircle<0 && relativeSpeedY>0 && speedX>=maxSpeed && m<15 && relativeAngleMovement<0.01){
				steer = (relativeAngleMovement<-0.01 || lastRelativeAngleMovement>=0.01) ? -turn : steer;
				hazard = 2;
			}
			if (relativeAngle<-0.001 && relativePosMovement<-0.01 && toOutterEdge<=GAP)
				...
				steer = (distToEstCircle>0 && relativeAngleMovement>0) ? steer : relativeAngle>0 && absSpeedY<MODERATE_SPEEDY && absSpeedY<absLastSpeedY && (seenNewSeg || trSz>1 && last_speed>first_speed 
						|| lastAcc>CONSTANT_SPEED_ACC && speedX<last_speed+m*2 || speedX<last_speed+Math.max(m,tW) && distToEstCircle>-tW) ? maxAbs(steer,maxAbs(lastSteer, steer)+turn*0.06) 
									: (relativeSpeedY>0) 
									? speedX>lowestSpeed && relativeAngleMovement<-0.01 ? -turn : steer 
								: steer;
				hazard = 2;
			} else if (speedX>lowestSpeed-15 && steer*turn<0 && relativeSpeedY>=0 && b>TURNANGLE*0.75 && a>TURNANGLE*0.5 && m<20 && relativePosMovement>0 && distToEstCircle<-GAP)
				steer = (relativePosMovement>0.03) ? (relativeAngleMovement>0.001) ? 0 : steer*0.5 : -turn;
				...
				steer = (absSpeedY>MODERATE_SPEEDY || relativeAngleMovement<-0.01) ? -turn : (steer*lastSteer>=0) ? maxAbs(lastSteer,steer*1.5):steer*1.5;
				...
				if (m<10 && relativePosMovement>0.01 && distToEstCircle>0 && relativeAngleMovement>-0.01){
				...
				} else if (speedX<lowestSpeed-15 && distToEstCircle<0 && lastDistToEstCircle<0 && !canGoToLastSeg && (lastDistToEstCircle>distToEstCircle  || b>TURNANGLE && a>TURNANGLE*0.5 && distToEstCircle<-GAP))
					steer = (relativeAngleMovement<-0.01) ? -turn : relativeSpeedY<LOW_SPEEDY || speedX<lowestSpeed && distToEstCircle>-W 
							? (distToEstCircle<0) 
									? -turn*a*0.5/steerLock
									: maxAbs(steer, lastSteer) : (m<10 && distToEstCircle>0) ? maxAbs(steer, lastSteer)-turn*0.06 : -turn;
					...
					steer = (distToEstCircle>lastDistToEstCircle && (m>15 || !isFast)) ? (toInnerEdge<-GAP) ? steer : 0 : 
						(m<10 && isFast && speedX>highestSpeed+tW*2 && (toInnerEdge<-GAP || distToEstCircle<0)) 
							? (distToEstCircle<0 || !canGoVeryFast && !canGoModerate) ? -turn : steer 
							: (relativeAngleMovement>0.01 && distToEstCircle>-GAP) 
								? 0	: (steer*lastSteer>0) ? maxAbs(steer,lastSteer) : steer;
					...
					if (distToEstCircle<0 && !canGoVeryFast && (relativeAngleMovement<-0.01 || distToEstCircle<lastDistToEstCircle || b>TURNANGLE && speedX>maxSpeed+10)){
						steer = (relativeSpeedY>0 || relativeAngleMovement<-0.02) ? -turn : maxAbs(steer, lSteer)*0.5;
						hazard = 2;
					} else steer =(relativeAngleMovement<-0.001 && distToEstCircle<lastDistToEstCircle && distToEstCircle<0 && (speedX>=maxSpeed+10 || distToEstCircle<-GAP) || relativeAngleMovement<-0.01 && (relativeAngle<0.001 || relativeSpeedY>LOW_SPEEDY)) 
								? (relativeAngleMovement<-0.01 || distToEstCircle<-GAP) 
									? -turn 
									: (relativeAngleMovement<-0.001) 
										? steer 
										: (relativeAngleMovement<0.01) 
											? steer*0.5 
											: (relativeAngleMovement>0.02) ? 0 : 0
								: lastSteer*steer<0 
									? steer 
									: (relativeSpeedY>0) ? steer : steer*0.5;
				}else if (steer*turn<0 && relativeSpeedY>0 && speedX>faster(last_speed,m) && distToEstCircle<0 && m<30 && b>TURNANGLE){
					steer = relativeAngleMovement<0.01 && (distToEstCircle<-GAP || relativePosMovement<0.02 && relativeAngleMovement<0) 
						? -turn 
						: steer;//CHU Y VAO DAY
//					hazard = 2;
					...
					steer = (m<=20 || distToEstCircle<-GAP || distToEstCircle<0 && distToEstCircle<lastDistToEstCircle) && (distToEstCircle<0 || distToEstCircle<GAP && distToEstCircle<lastDistToEstCircle && relativeAngleMovement<-0.001) 
						? (canGoVeryFast || relativeAngleMovement>0.005 || relativeAngleMovement>lastRelativeAngleMovement && distToEstCircle>-GAP*0.5 && relativeAngleMovement>0.001) ? steer : -turn 
						: (relativeAngleMovement>0.01) 
							? m>10 ? 0 : steer 
...
//			if (a<0 && relativePosMovement<-0.001 && steer*turn>0 && m<10) 
//				steer =  relativeAngleMovement>0.01 && relativeAngleMovement>lastRelativeAngleMovement || relativeSpeedY<0 && relativeAngleMovement>-0.001 ? 0 : -turn;
...
//		if (a==0 && turn!=0 && turn!=2 && steer*turn<0 && relativeSpeedY<0 && speed<lowestSpeed-15 && relativePosMovement>-0.001) 
...
//					speedX<220 && hl>60 && (al>55 || relativePosMovement>-0.001) ||
					...
					speedX<220 && hl>60 && (al>60 || relativePosMovement>-0.001) ||
					speedX<200 && hl>50 && hl>al && (al>45 || relativePosMovement>-0.001) ||
					speedX<195 && hl>48 && hl>al && (al>45 || relativePosMovement>-0.001) ||
		...
		if (!canGoVeryFast){
			if (hl>80 && (al>30 || relativePosMovement>-0.001) && relativeTargetAngle<0.35 ||
				...
				speedX<200 && hl>50 && (al>40 || relativePosMovement>-0.001) ||
				speedX<195 && hl>48 && (al>40 || relativePosMovement>-0.001) ||
...
//				combine(first, left, sL, right,sR);
		...
		if (turn==2){
			turn = edgeDetector.whichE==0 && hl<EdgeDetector.MAX_DISTANCE ?  2 : (!inTurn && last!=null && last.type==0 && lSeg.endIndex>=edgeDetector.lSize-1 && rSeg.endIndex>=edgeDetector.rSize-1) 
					? (Math.abs(lSeg.end.x-lSeg.start.x)<TrackSegment.EPSILON*0.1) ? 0 : (lSeg.start.x<lSeg.end.x) ? 1 : -1
//					? 0
					: (edgeDetector.whichE==1) ? -1 : 1;
			edgeDetector.turn = turn;
			if (turn==0) {
				recording = false;						
				return;
...
//			turn = edgeDetector.turn;
			...
			if (inTurn && first!=null && first.type==0 && (l0.num>1 || r0.num>1) && Math.abs(first.start.x-first.end.x)<E) inTurn = false;
			if (inTurn && (l!=null || r!=null)){			
				if ((l!=null && l0!=null && l0.type==0 && l0.num>1 && l0.end.y>=1 && Math.abs(l0.end.x-l0.start.x)<E) 
						|| (r!=null && r0!=null && r0.type==0 && r0.num>1 && r0.end.y>=1 && Math.abs(r0.end.x-r0.start.x)<E)) {
					inTurn = false;
...
//			int correctTurn = signD<0 ? -1 : 1;
			...
			if (toInnerEdge<0 && relativeAngle<-0.5 && relativeAngleMovement<-0.025 || toInnerEdge>0 && relativeAngle>0.5 && relativeAngleMovement>0.025){
				gear = -1;
				steer = -signD;
				brake = 0;
			...
			if (absSpeedY<HIGH_SPEEDY){
				...
				if (turn!=2 && turn!=Segment.UNKNOWN){
					if (tp==0)
						...
						? relativeAngleMovement>0.03 ? (relativeAngle<-0.15) ? 0 : -signD : relativeAngleMovement>0.02 ? 0 : relativeAngleMovement>0.01 ? deflect : signD 
...
//						}
						...
						steer = outOfTrackSteering(signD, deflect, bal, absSpeedY);
					} else steer = (relativeAngle<-0.15 && relativeAngleMovement<-0.001) ? -signD : (relativeAngle<-0.1) ? bal*0.5 :(relativeAngle<-0.05 || absSpeedY>=MODERATE_SPEEDY && relativeAngle<0) ? (speedX>150 && relativePosMovement<-0.001) ? 0 : deflect*0.5 : lastSteer*deflect>0 && relativeAngle>=0 
							? relativeAngleMovement<-0.02 ? -signD : relativeAngleMovement<-0.01 ? 0
							: relativeAngle<0 ? bal*0.5 : relativeAngle<0.1 && relativeAngleMovement<-0.001 ? 0 : maxAbs(lastSteer, deflect) 
							: (relativePosMovement<-0.001) ? relativeAngleMovement<-0.01 || relativeAngle<0 && toInnerEdge<GAP ? -signD : 0 : deflect*0.5;
				} else {
													...
													:(relativeAngle>0.2) 
																...
																: (toOutterEdge<-GAP && (relativeAngleMovement<-0.001  || relativeAngleMovement<lastRelativeAngleMovement )) 
																	...
																	: relativePosMovement>0 
																		? toInnerEdge<0 
																				...
																				: relativeAngleMovement<-0.02 
																				? -signD : relativeAngleMovement<-0.01 ? 0 : relativeAngleMovement>0.001 ? deflect*0.5 : bal*0.5 
														...
														: (relativeAngle>0 && (relativeAngle<0.2 || relativeAngleMovement<-0.001)) 
															? (relativeAngle<0.3 && relativeAngleMovement<-0.01 && toOutterEdge<-GAP) 
																...
																: (relativeAngleMovement<-0.001) ?  toInnerEdge<0 ? signD 
																		: relativePosMovement<-0.001 || relativeAngleMovement<-0.01 ? bal*0.5 : 0 
																		: stableSteer(deflect) 
...
//			double relativeSpeedY = tp*speedY;
		...
		if (turn==STRAIGHT){
			...
			steer = (absSpeedY>=MODERATE_SPEEDY && relativeAngle>0 || relativeAngle>0.15) 
					? (absSpeedY>MODERATE_SPEEDY && relativeAngle<0.15) ? 0 :  bal*0.5					
					: lSteer+deflect;
			
//			if (distToEstCircle<-W && relativePosMovement<-0.02 && relativeAngleMovement<-0.005) acc = Math.min(acc,INCREASE_ONE);
...
//			smoothSteering();
		...
		if (turn==UNKNOWN){
			...
			steer = (relativeAngle<-0.05 && relativePosMovement<-0.02 && (relativeAngleMovement<-0.025 || relativeAngleMovement<-0.01 && lastRelativeAngleMovement<-0.01 && relativeAngleMovement<lastRelativeAngleMovement)) 
					? Math.signum(lSteer)
					: (absSpeedY>10) ? lSteer : (absSpeedY<LOW_SPEEDY) ? lSteer + offset*0.75: lSteer + offset;
//			if (isOffBalance) {
//				System.out.println();
//				steer *=0.1;
//				steer = 0;

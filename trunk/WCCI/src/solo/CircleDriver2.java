...
public final class CircleDriver2{
	 ...
	 * 
	 */
	public static final double BREAK_TIME = 4200.08;
...
//				if (speedX<aheadSpeed+al*0.5 && (absSpeedY<MODERATE_SPEEDY || a<=0|| relativeAngleMovement>0.01) ) acc = 1;
				...
				if (a>0 && b>0){
					if (brake>0 && absSpeedY<HIGH_SPEEDY && (inTurn || seenNewSeg)) 
						brake = (distToEstCircle<-GAP) ? brake*0.5 : 0;					
...
//				trackData.add(ts);
	...
	private double danger(double a,double b,double absSpeedY,double steer,double lSteer){
		if (distToEstCircle<-GAP && b>TURNANGLE*0.5 || b>TURNANGLE*0.75 && distToEstCircle<0)
			...
			return (distToEstCircle<0) ? lSteer : (a>TURNANGLE/1.5 && absSpeedY<MODERATE_SPEEDY) ? (b>TURNANGLE) ? lSteer : lSteer*0.5 
					: b>TURNANGLE || a>TURNANGLE*0.5 && distToEstCircle<GAP ? lSteer : 0;

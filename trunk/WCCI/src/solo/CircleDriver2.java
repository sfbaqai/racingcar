...
public final class CircleDriver2 extends BaseStateDriver<NewCarState,CarControl> {
	...
	public static final double ERR = 0.1;
	...
	static double[] ar = new double[1000];
	...
	private static ObjectList<Segment> l = null;
	private static ObjectList<Segment> r = null;
	...
	Segment curSeg = null;
	private static LinkedList<Segment> temp = new LinkedList<Segment>();
	int tIndex = -1;
	int count = -1;
	double rDist = -1;
	public static Double2ObjectSortedMap<Vector2D> allL = new Double2ObjectRBTreeMap<Vector2D>();
	public static Double2ObjectSortedMap<Vector2D> allR = new Double2ObjectRBTreeMap<Vector2D>();
	private static ObjectList<Segment> lm = new ObjectArrayList<Segment>();
	private static ObjectList<Segment> rm = new ObjectArrayList<Segment>();
	...
	private static ObjectList<Segment> lTmp =null;
	private static ObjectList<Segment> tE = null;
	...
	public void estimatePath(Vector2D hh){
		...
		if (centerOfTurn!=null){
			...
			if (isTurning){
				...
				if (mustPassPoint.y<3) 
					mustPassPoint = point;
				double[] r = new double[3];
				if (detected && l.get(l.size()-1).type==Segment.UNKNOWN){
					Vector2D v = l.get(l.size()-1).end;					
					cntr = Geom.getCircle3(new Vector2D(0,0), carDirection, v, edgeDetector.highestPoint, r);
					rr = Math.sqrt(r[2])-1;
				} else if (tr.size()==1){
					if (point!=null && Geom.getCircle2(mustPassPoint, optimalPoint, new Vector2D(0,0), carDirection, r)!=null){				
						...
						Vector2D n = carDirection.orthogonal();					
						rr = Math.sqrt(r[2])-1;
						Geom.getLineLineIntersection(o.x, o.y, o.x+n.x, o.y+n.y, 0, 0, carDirection.x, carDirection.y, r);
				...
				} else if (inTurn && curSeg.type!=0 && tr.get(tr.size()-1).type!=curSeg.type){
					//						Segment last = tr.get(tr.size()-1);
					Vector2D v = l.get(l.size()-1).end;
					//						Vector2D v0 = l.get(l.size()-1).start;
					cntr = Geom.getCircle3(new Vector2D(0,0), carDirection, edgeDetector.currentPointAhead,v, r);
					rr = Math.sqrt(r[2])-1;
				}
	...
	public void check(AffineTransform at,ObjectList<Segment> l,ObjectArrayList<Vector2D> all){
		...
		for (k=0;k<l.size();) {
			Segment ll = l.get(k);
			ll.transform(at);
			if (ll.end!=null && ll.end.y<0){
				l.remove(k);
				continue;
			}
			for (int kk = 0;kk<ll.num;++kk){
				Vector2D v = ll.points[kk];
				int index = binarySearchFromTo(all.elements(), v, 0, all.size()-1, Segment.comp);
				if (index<0){
					index = -index -1;					
					boolean ok = false;
					boolean found = false;
					...
					if (index>0){
						p = all.get(index-1);
						int pos = binarySearchFromTo(ll.points, p, 0, ll.num-1, Segment.comp);
					...
					if ((k>0 && found) || (k==0 && ll.type==0 && ll.num>2)) ll.removePoint(v);
					if (ll.start==null) {
						l.remove(k);
						--k;
						break;
...
//								ll.num++;
								...
								ll.reCalculate(tW);
								if (k==1 && l.get(0).type==0 && (Double.isNaN(ll.radius) || Math.abs(oldr-ll.radius)>0.5)){
									double cx = ll.center.x;							
									double x0 = l.get(0).start.x;
									double lx = cx-ll.radius-x0;
									double rx = cx+ll.radius-x0;
									if (lx>ERR || rx<-ERR || (lx*rx<0 && (lx<-ERR && rx>ERR))){
										double[] rr = new double[3];
										Geom.getCircle2(ll.start, ll.end, new Vector2D(x0,0), new Vector2D(x0,1), rr);					
										rr[2] = Math.sqrt(rr[2]);
										ll.radius = (int)(Math.round(rr[2]-trackWidth*0.5)+trackWidth*0.5);
										ll.center = new Vector2D(rr[0],rr[1]);
										ll.seg = TrackSegment.createTurnSeg(ll.center.x, ll.center.y, ll.radius, ll.start.x, ll.start.y, ll.end.x, ll.end.y);										
										ll.length = ll.seg.length;
										ll.type = ll.seg.type;
									}
								}
								int rrr = Segment.double2int(ll.radius);
								ll.map[rrr]++;
								if (ll.minR<rrr) ll.minR = rrr;
								if (ll.maxR>rrr) ll.maxR = rrr;
		...
		for (Segment ll:l) map.addAll(ObjectArrayList.wrap(ll.points,ll.num));		
			...
			for (Segment ll : l){		
				...
				if (ll.type!=Segment.UNKNOWN && isBelong){
					...
					if (ll.type!=0 && ll.num<=Segment.LIM){
						double oldr = ll.radius;
						ll.reCalculate(tW);
						if (j==1 && l.get(0).type==0 && (Double.isNaN(ll.radius) || Math.abs(oldr-ll.radius)>0.5)){
							double cx = ll.center.x;							
							double x0 = l.get(0).start.x;
							double lx = cx-ll.radius-x0;
							double rx = cx+ll.radius-x0;
				...
				if (ll.type!=0 && ll.center!=null){								
					if (ll.type!=Segment.UNKNOWN){
						ll.addPoint(v);												
						Vector2D[] vv = ll.points;
						int len = ll.num;
						double oldr = ll.radius;
						double r = 0;
						Vector2D center = null;
						if (j==1 && l.get(0).type==0){													
							double x0 = l.get(0).end.x;							
							double[] rr = new double[3];
							Geom.getCircle2(ll.start, ll.end, new Vector2D(x0,0), new Vector2D(x0,1), rr);					
							rr[2] = Math.sqrt(rr[2]);
							r = (int)(Math.round(rr[2]-trackWidth*0.5)+trackWidth*0.5);
							center = new Vector2D(rr[0],rr[1]);
						} else {
							CircleFitter cf = new CircleFitter(new double[]{ll.center.x,ll.center.y,ll.radius},ll.points,0,ll.num-1);
							cf.fit();
							r = Math.round(cf.getEstimatedRadius()-trackWidth*0.5)+trackWidth*0.5;
							center = cf.getEstimatedCenter();
						}
						
						if (Math.abs(r-ll.radius)<1){
							Vector2D p = vv[0].plus(vv[len-1]).times(0.5);
							Vector2D n = vv[len-1].minus(vv[0]).orthogonal().normalized();
							if (n.dot(center.minus(p))<0) n = n.negated();
							double dd = vv[0].distance(p);
							if (r<=dd) r = ll.radius;
							center = p.plus(n.times(Math.sqrt(r*r-dd*dd)));
							found = true;
							for (int ii = 0;ii<ll.num;++ii){
								Vector2D vvv = ll.points[ii];
							...
							if (found) {								
								ll.center = center;
								int rrr = Segment.double2int(r); 
								ll.radius = (int)r;
								ll.map[rrr]++;
								if (ll.minR>rrr) ll.minR = rrr;
								if (ll.maxR<rrr) ll.maxR = rrr;
								ll.start = new Vector2D(ll.points[0]);
								ll.end = new Vector2D(ll.points[len-1]);
								ll.seg = TrackSegment.createTurnSeg(ll.center.x, ll.center.y, ll.radius, ll.start.x, ll.start.y, ll.end.x, ll.end.y);
								ll.reCalLength();
								break;						
							}
						...
						ll.removePoint(v);
					} else if (isIn && ll.type!=Segment.UNKNOWN && ll.num>Segment.LIM){
						found = true;
						break;
					}
				}
				if (v.y<=ll.end.y || j==l.size()-1){					
					...
					for (k=j;k>=0;k--){
						...
						mp.addAll(ObjectArrayList.wrap(s.points,s.num));
					}
					if (k>=0 && v.y>l.get(k).end.y) 
						k++;					

					mp.add(v);
					for (m=j+1;m<l.size();++m){
						Segment s = l.get(m);
						if (s.type!=Segment.UNKNOWN && s.num>3) break;
						mp.addAll(ObjectArrayList.wrap(s.points,s.num));
					}
					...
					if (k==m && isIn){
						m++;
						mp.addAll(ObjectArrayList.wrap(ll.points,ll.num));
					}
					if (mp.size()>0) Segment.sortPoints(mp);
					if (k>=0 && k<m) l.removeElements(k, m);

					List<Segment> ol = Segment.segmentize(mp.elements(),0,mp.size(),tW);
					if (k>=0) {
						for (Segment ss:ol){
							if (ss.type!=Segment.UNKNOWN){
								int rrr = Segment.double2int(ss.radius);
								ss.map[rrr]++;
								if (ss.minR>rrr) ss.minR = rrr;
								if (ss.maxR<rrr) ss.maxR = rrr;
							}
						}
						l.addAll(k, ol);
						Segment.lastCheck(l, tW);
...
//			List<Segment> ol = Segment.segmentize(smt.points,0,smt.num, tW);
//			l.remove(l.size()-1);
//			l.addAll(ol);
//		}
		System.out.println("Time Check: "+(System.currentTimeMillis()-ti));

	}

	public void fix (ObjectList<Segment> l,double sD){//sD is straight Dist
		if (l!=null && l.size()>1 && l.get(0).type==0){
			Segment l0 = l.get(0);
			if (l0.end.y<sD){
				boolean found = false;
				for (int i=1;i<l.size();++i){
					Segment s = l.get(i);
					Vector2D[] ar = s.points;
					int len = s.num;
					for (int j=0;j<len;++j){
						Vector2D v = ar[j];
						if (v!=null && v.y<=sD){
	...
	public void record(CarState cs){		
		...
		ObjectArrayList<Vector2D> left = EdgeDetector.left;
		ObjectArrayList<Vector2D> right = EdgeDetector.right;
		...
		if (inTurn){
			l = (left==null) ? null : Segment.segmentize(left, tW);
			r = (right==null) ? null :Segment.segmentize(right, tW);			
		}
		...
		if (!inTurn && l!=null && r!=null && left!=null && right !=null && l.size()>0 && r.size()>0){
			...
			if (l0.type!=0 && r0.type!=0)
				...
				ll = Segment.segmentize(left, tW);
				rrr = Segment.segmentize(right, tW);
			...
			if (inTurn){											
				l = (ll==null) ? Segment.segmentize(left, tW) : ll;
				r = (rrr==null) ? Segment.segmentize(right, tW) : rrr;
			}
		}


		if (!inTurn && (prevEdge.straightDist<=0 || nraced<=0 || nraced<prevEdge.straightDist-0.5 || (prevEdge.center!=null && prevEdge.center.y>=nraced))) {				
			AffineTransform at = edgeDetector.combine(prevEdge, nraced);
			System.out.println("Here : "+(System.currentTimeMillis()-ti));
			if (tr!=null)
			for (Segment tt:tr)
				tt.transform(at);
...
//					check(at, r, right);
				r = Segment.segmentize(right.elements(), 0, right.size(), 1, tW, tr, distRaced);
//				}
			}
//			if (time >=12.27 && tr!=null){
//				@SuppressWarnings("unused")
//				ObjectList<Segment> oal = Segment.segmentize(left.elements(), 0, left.size(), -1, tW, tr, distRaced);
//				@SuppressWarnings("unused")
//				ObjectList<Segment> oal1 = Segment.segmentize(right.elements(), 0, right.size(), 1, tW, new ObjectArrayList<Segment>(), distRaced);
//				System.out.println();
//			}
			inTurn = false;
		}		
		...
		if (tr!=null && tr.size()>0 && tr.get(0).type==0 && sD>0){
			Segment t = tr.get(0);
			edgeDetector.straightDist = Math.max(sD, t.dist+t.length-distRaced);
			sD = edgeDetector.straightDist;
		}


		Segment l0 = (l!=null && l.size()>0) ? l.get(0) : null;
		Segment r0 = (r!=null && r.size()>0) ? r.get(0) : null;
		if (sD>0){
			if (l0!=null && l0.type==0) sD = Math.max(sD, l0.end.y);
			if (r0!=null && r0.type==0) sD = Math.max(sD, r0.end.y);
		}
...
//		rm = ObjectArrayList.wrap(segArr, 0);
		lm = new ObjectArrayList<Segment>();
		rm = new ObjectArrayList<Segment>();
...
//				}
		...
		System.out.println("Time after store: "+(System.currentTimeMillis()-ti));
		...
		guessTurn();
		System.out.println("Time after guessturn: "+(System.currentTimeMillis()-ti));
		...
		prevEdge = new EdgeDetector(edgeDetector);				
		raced = distRaced;
		...
		if (inTurn){
			...
			if (tr!=null && tr.size()>0){
				...
				if (!ok){
					...
					if (s.num>=1 && edgeDetector.highestPoint.length()<99.95 && ((which==-1 && l.size()>0 && s==l.get(l.size()-1)) || (which==1 && r.size()>0 && s==r.get(r.size()-1)) )){ 
						...
						if (s.num>=2 && t.type==0){
							...
							if (rr[2]<1000){						
								...
								s.radius = (int)Math.round(edgeRadius-turn*which*trackWidth*0.5);
								double d = s.radius*s.radius;
								if (s.map==null) s.map = new int[Segment.MAX_RADIUS]; 
								int rad = Segment.double2int(s.radius); 
								s.map[rad] = 1;
								s.minR = rad;
								s.maxR = rad;
...
//	}
	...
	public double steerAtRadius(CarState state,double cx,double cy,double targetRadius){
		...
		if (d<targetRadius){	
			Vector2D p = point.normalised();
			point = new Vector2D().plus(p.times(Math.sqrt(targetRadius*targetRadius-d*d)));
			return gotoPoint(state, p);
	...
	public final void storeTrack(EdgeDetector ed,CarState cs,List<Segment> ol,List<Segment> or){
		long ti = System.currentTimeMillis();
		...
		analyze(or,0);
		System.out.println("Time Store begin "+(System.currentTimeMillis()-ti));
...
//		tE = ObjectArrayList.wrap(nsegArr, 0);
		lTmp = new ObjectArrayList<Segment>();
		tE = new ObjectArrayList<Segment>();
		System.out.println("Time Store start "+(System.currentTimeMillis()-ti));
...
//			System.out.println("Time Store "+i+" 0: "+(System.currentTimeMillis()-ti));
			...
			while (ri<or.size()){				
				...
				if (s.dist<=d && s.dist+s.length>=e){
					if (tmp==null){
						tmp = new Segment(s);
//						System.out.println("Time Store "+i+" 1 : "+(System.currentTimeMillis()-ti));
						tmp.dist = d;
						tmp.length = e-d;
						tmp.arc = tmp.type*tmp.length/tmp.radius;
						which = 1;
						...
						if (tmp.type==s.type || s.type==0 || tmp.type==0){
							for (int r = s.minR;r<=s.maxR;++r){
								if (s.map[r]==0) continue;
								int nr = s.map[r];
								nr += tmp.map[r];
								tmp.map[r] = nr;
								if (r<tmp.minR) tmp.minR = r;
								if (r>tmp.maxR) tmp.maxR = r;
								if (nr>maxn){
									dmax = r;
									maxn = nr;
								}
...
//			System.out.println("Time Store "+i+" 1b : "+(System.currentTimeMillis()-ti));
						...
						if (tmp==null){
							...
							if (tmp.type==s.type || s.type==0 || tmp.type==0){
								...
								if (dmax!=Segment.double2int(tmp.radius)){							
									...
									if (dmax<0){
										tmp.type = -tmp.type;										
										for (int r = tmp.minR;r<tmp.maxR;++r){
											if (tmp.map[r]==0) continue;
											tmp.map[r] = -tmp.map[r];
										}										
									}
			...
			//			System.out.println("Time Store"+i+": "+(System.currentTimeMillis()-ti));
		}//end of for

		if (tE!=null && tE.size()>0) lTmp.addAll(tE);		
		if (tr!=null && tr.size()>0) {
			tr.removeElements(tIndex, k);			
			tr.addAll(tIndex,lTmp);						
		} else tr = new ObjectArrayList<Segment>(lTmp);
		analyze(tr, tIndex);
		System.out.println("Time Store: "+(System.currentTimeMillis()-ti));
	};

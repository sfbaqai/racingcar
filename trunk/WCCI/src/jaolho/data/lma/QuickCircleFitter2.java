...
public final class QuickCircleFitter2 extends QuickLMA<Vector2D> {	
	 ...
	 * @param to
	 */
	public QuickCircleFitter2(double[] parameters, Vector2D[] x, double[] y,
			double[] weights, int from, int to) {
		super(parameters, x, y, weights, from, to);
		// TODO Auto-generated constructor stub
	}
	
	public QuickCircleFitter2(double[] parameters, Vector2D[] x, int from, int to) {		
		super(parameters, x, new double[to], from, to);
		
	}
	
	public QuickCircleFitter2(double[] parameters, Vector2D[] x) {		
	...
	private double getAverageR(double[] params){
		...
		double totalX = 0;
		double totalY = 0;
		double param0 = params[0];
		
		for (int i=from;i<to;++i){
			double x  = param0-xDataPoints[i].x;
			double y  = -xDataPoints[i].y;
			double d = Math.sqrt(x*x+y*y);
			totalX +=  x/d;
			totalY +=  y/d;
			total +=  d-yDataPoints[i];
		}
		avrgR = total/size;
		dJdx = totalX/size;
		dJdy = totalY/size;
...
//		case 1:return dy/d;
	...
	public final Vector2D getEstimatedCenter(){
		return new Vector2D(parameters[0],0);
	}

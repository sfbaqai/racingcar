...
public abstract class AbstractBooleanList extends AbstractBooleanCollection implements BooleanList , BooleanStack {
 ...
 public static class BooleanSubList extends AbstractBooleanList implements java.io.Serializable {
  ...
  public final boolean addAll( final int index, final Collection<? extends Boolean> c ) {
   int sz = to - from;
   if ( index < 0 ) throw new IndexOutOfBoundsException( "Index (" + index + ") is negative" );
   if ( index > sz ) throw new IndexOutOfBoundsException( "Index (" + index + ") is greater than list size (" + ( sz ) + ")" );
   to += c.size();
  ...
  public final void addElements( int index, final boolean a[], int offset, int length ) {
   int sz = to - from;
     if ( index < 0 ) throw new IndexOutOfBoundsException( "Index (" + index + ") is negative" );
     if ( index >= sz ) throw new IndexOutOfBoundsException( "Index (" + index + ") is greater than or equal to list size (" + ( sz ) + ")" );
  ...
  public BooleanListIterator listIterator( final int index ) {
   ...
   return new AbstractBooleanListIterator () {
     ...
     public final boolean nextBoolean() { if ( pos >= size() ) throw new NoSuchElementException(); return l.getBoolean( from + ( last = pos++ ) ); }
     public final boolean previousBoolean() { if ( pos <= 0 ) throw new NoSuchElementException(); return l.getBoolean( from + ( last = --pos ) ); }
     public final int nextIndex() { return pos; }
     public final int previousIndex() { return pos - 1; }
     public final void add( boolean k ) {
      if ( last == -1 ) throw new IllegalStateException();
      BooleanSubList.this.add( pos++, k );
      last = -1;
      if ( ASSERTS ) assertRange();
     }
     public final void set( boolean k ) {
      if ( last == -1 ) throw new IllegalStateException();
  ...
  public final BooleanList subList( final int from, final int to ) {

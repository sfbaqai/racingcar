...
public class ShortArrayFrontCodedList extends AbstractObjectList<short[]> implements Serializable, Cloneable {
 ...
 public ShortArrayFrontCodedList( final Iterator<short[]> arrays, final int ratio ) {
  ...
  short[] array = ShortArrays.EMPTY_ARRAY;
  ...
  while( arrays.hasNext() ) {
   ...
   if ( n % ratio == 0 ) {
    ...
    array = ShortArrays.grow( array, curSize + count( length ) + length, curSize );
    ...
    array = ShortArrays.grow( array, curSize + count( length ) + count( common ) + length, curSize );
    curSize += writeInt( array, length, curSize );
  ...
  this.ratio = ratio;
  this.array = ShortArrays.trim( array, curSize );
 ...
 /** Reads a coded length.
	 ...
	 */
 private static int readInt( final short a[], int pos ) {
 ...
 /** Writes a length.
	 ...
	 * @return the number of elements coding <code>length</code>.
	 */
 private static int writeInt( final short a[], int length, int pos ) {
  if ( length < ( 1 << 15 ) ) {
   a[ pos ] = (short)length;
   return 1;
  }
 ...
 public ObjectListIterator<short[]> listIterator( final int start ) {
  ...
  return new AbstractObjectListIterator<short[]>() {
    short a[] = ShortArrays.EMPTY_ARRAY;
    int i = 0, pos = 0;
    ...
    public short[] next() {
     ...
     if ( i % ratio == 0 ) {
      pos = p[ i / ratio ];
      length = readInt( array, pos );
      a = ShortArrays.ensureCapacity( a, length, 0 );
      ...
      if ( inSync ) {
       length = readInt( array, pos );
       common = readInt( array, pos + count( length ) );
       a = ShortArrays.ensureCapacity( a, length + common, common );
       System.arraycopy( array, pos + count( length ) + count ( common ), a, common, length );
       pos += count( length ) + count( common ) + length;
       length += common;
      }
      else {
       a = ShortArrays.ensureCapacity( a, length = length( i ), 0 );
       extract( i, a, 0, length );
      }
     }
     i++;
     return ShortArrays.copy( a, 0, length );
    }
 ...
 public Object clone() {
  ShortArrayFrontCodedList c;
  try {
   c = (ShortArrayFrontCodedList)super.clone();
  }

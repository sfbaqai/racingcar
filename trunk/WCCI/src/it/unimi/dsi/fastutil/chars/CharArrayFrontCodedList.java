...
public class CharArrayFrontCodedList extends AbstractObjectList<char[]> implements Serializable, Cloneable {
 ...
 public CharArrayFrontCodedList( final Iterator<char[]> arrays, final int ratio ) {
  ...
  while( arrays.hasNext() ) {
   ...
   if ( n % ratio == 0 ) {
    ...
    array = CharArrays.grow( array, curSize + count( length ) + length, curSize );
    ...
    array = CharArrays.grow( array, curSize + count( length ) + count( common ) + length, curSize );
    curSize += writeInt( array, length, curSize );
 ...
 /** Reads a coded length.
	 ...
	 */
 private static int readInt( final char a[], int pos ) {
 ...
 /** Writes a length.
	 ...
	 * @return the number of elements coding <code>length</code>.
	 */
 private static int writeInt( final char a[], int length, int pos ) {
  ...
  return new AbstractObjectListIterator<char[]>() {
    ...
    public char[] next() {
     ...
     if ( i % ratio == 0 ) {
      pos = p[ i / ratio ];
      length = readInt( array, pos );
      a = CharArrays.ensureCapacity( a, length, 0 );
      ...
      if ( inSync ) {
       length = readInt( array, pos );
       common = readInt( array, pos + count( length ) );
       a = CharArrays.ensureCapacity( a, length + common, common );
       System.arraycopy( array, pos + count( length ) + count ( common ), a, common, length );
       pos += count( length ) + count( common ) + length;
       length += common;
      }
      else {
       a = CharArrays.ensureCapacity( a, length = length( i ), 0 );
       extract( i, a, 0, length );
      }
     }
     i++;
     return CharArrays.copy( a, 0, length );
    }
